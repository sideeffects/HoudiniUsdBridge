...
getHdFormat(const PXL_DataFormat format, const PXL_Packing packing)
{
	...
	case PXL_INT16:
	    switch (packing)
	    {
		case PACK_SINGLE:	return HdFormatUInt16;
		case PACK_UV:		return HdFormatUInt16Vec2;
		case PACK_RGB:		return HdFormatUInt16Vec3;
		case PACK_RGBA:		return HdFormatUInt16Vec4;
		default:		break;
	    }
	    break;
	    return HdFormatInvalid;
...
BRAY_HdAOVBuffer::GetResource(bool) const
{
    ...
    if (myAOVBuffer.getNumExtra() <= 0)
        return VtValue(myAOVBuffer);
#else

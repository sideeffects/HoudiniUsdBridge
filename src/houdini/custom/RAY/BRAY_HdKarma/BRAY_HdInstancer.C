...
namespace
{
            ...
            [&](const UT_BlockedRange<exint> &r)
            {
                ...
                for (exint i = r.begin(), n = r.end(); i < n; ++i)
                {
                    ...
                    if (idx < 0 || idx >= datasize)
                        continue;
                    const V3	&x0 = seg0[idx];

                    if (DO_INTERP)
                    {
                        const V3	&x1 = seg1[idx];
        ...
        UTparallelFor(UT_BlockedRange<exint>(0, transforms.size()),
            [&](const UT_BlockedRange<exint> &r)
            {
                ...
                for (exint i = r.begin(), n = r.end(); i < n; ++i)
                {
                    ...
                    if (idx < 0 || idx >= datasize)
                        continue;
                    const M4	&x0 = seg0[idx];
                    if (DO_INTERP)
                    {
                        // TODO: Better interpolation
                        const M4 &x1 = seg1[idx];
	...
	METHOD(UT_Array<GfMatrix4d> &transforms, \
                const VtIntArray &instanceIndices, \
		const V *primvar0, const V *primvar1, exint primvar_size, float lerp) \
	{ \
	    if (primvar0 != primvar1 && lerp != 0) { \
		IMPL<V, true>(transforms, instanceIndices, \
			primvar0, primvar1, primvar_size, lerp); \
	    } else { \
		IMPL<V, false>(transforms, instanceIndices, \
			primvar0, primvar1, primvar_size, 0); \
	    } \
	} \
	/* end of macro */
	...
	if (!source)
	    return;
	static constexpr UT_StringLit  thePrefix("light:");
	UT_StringArray                 snames;
	GT_AttributeMapHandle          pmap;
	UT_SmallArray<int>             pidx;
	for (int i = 0, n = source->entries(); i < n; ++i)
	{
		...
		if (!pmap)
		    pmap = UTmakeIntrusive<GT_AttributeMap>();
                ...
                // Add the prefixed name to attrib map
                pidx.append(pmap->add(sname, false));
                UT_ASSERT(pidx.last() >= 0);
	    }
	}

	// Check if any light attribs have been found
	if (!snames.size())
	    return;

        // Currently, properties cannot be motion blurred
        attribs = UTmakeIntrusive<GT_AttributeList>(pmap, 1);
        for (int i = 0, n = snames.size(); i < n; ++i)
        {
            attribs->set(pidx[i], source->get(snames[i]));
        }
    }
	...
	BRAY::ScenePtr &scene)
{
    ...
    {
	// In this case, we have some geometry to instance,
	// If there are multiple geometries in our instance map,
	// we will pack them into a scene graph so we can easily
	// call update on it or pass it to the parent instancer
	// if needed.
	// Note that we can have both geometries and lights to instance,
	// and the lights are processed further down in the function.
	
	// Make sure to build the scene graph if required
	BRAY::ObjectPtr proto;
	if (myInstanceMap.size() > 1)
	{
	    // In this case, we have multiple objects being instanced.
	    // For this we want to aggregate the edits into a scene graph.
	    if (!mySceneGraph)
	    {
		myNewObject = true;
	...
	// At this point, proto will contain either the single object
	// that is being instanced, or a scene graph containing the multiple
	// objects that are being instanced.
    ...
    if (myLightInstanceMap.size())
    {
	// In this case we have some lights to instance
	// If there were also geometries being instanced,
	// they should be processed by this point.
	
	if (GetParentId().IsEmpty())
	{
	    // There is no nested instancing needed, so just
	    // commit the light instances in our map to the scene.
	    for (auto &&inst : myLightInstanceMap)
		inst.second.commit(scene);
	}
	else
	{
	    // In this case, there is nested instancing, so we pack
	    // all of our light instancers into one light instancer
	    // so we can pass it to the parent instancer for processing.

	    if (!myLightCollection)
	    {
		// If we don't already have myLightCollection, we create it
		const UT_StringHolder &name =
		    BRAY_HdUtil::toStr(GetId());
		myLightCollection = scene.createLightInstancer(name);
		UT_SmallArray<BRAY::SpacePtr> xforms;
		xforms.emplace_back(UT_Matrix4D::getIdentityMatrix());
		myLightCollection.setInstanceTransforms(scene, xforms);
	    }

	    // We add the instancers in our instance map as prototypes
	    // to the light instancer we are passing to the parent.
	    for (auto &&inst : myLightInstanceMap)
		myLightCollection.addPrototype(scene, inst.second);	

	    // We pass the light instancer containing all of our instances
	    // to the parent, similar to the case for instanced geometries.
	    HdInstancer	*parentInstancer =
		GetDelegate()->GetRenderIndex().GetInstancer(GetParentId());
	    UT_ASSERT(parentInstancer);
	    UT_SmallArray<GfMatrix4d>	px;
	    ...
	    UTverify_cast<BRAY_HdInstancer *>(parentInstancer)->
		NestedInstances(rparm, scene, GetId(),
			myLightCollection, px,
                        ...
                        HdDirtyBits *dirtyBits)
{
    ...
    {
        myVisible = sd->GetVisible(GetId());
    }

    if (HdChangeTracker::IsAnyPrimvarDirty(*dirtyBits, GetId()) ||
        HdChangeTracker::IsTransformDirty(*dirtyBits, GetId()))
    {
        syncPrimvars(sd, renderParam, dirtyBits);
    }
        ...
        HdDirtyBits* dirtyBits)
{
    ...
    {
        // We don't know what kind of attributes might be required by the
        // prototypes on the instance, so we keep all the instance primvars (we
        // don't limit based on material usage).
...
BRAY_HdInstancer::NestedInstances(BRAY_HdParam &rparm,
	...
	const BRAY::InstancablePtr &protoInstancable,
        ...
        const BRAY::OptionSet &protoProps)
{
    ...
    for (int i = 0; i < mySegments; ++i)
    {
        ...
        else
        {
            ...
            // TODO:  If there are more than mySegments transform, we could get
            // better motion by interpolating the proto transforms instead of
            // flooring.
            int	pidx = SYSrint((protoXform.size()-1)*shutter);
    ...
    // Light being instanced
    const BRAY::LightPtr    *protoLight =
	BRAY::dyn_cast<BRAY::LightPtr>(protoInstancable);

    // Light instancer being instanced
    const BRAY::LightInstancerPtr    *protoLightInstancer =
	BRAY::dyn_cast<BRAY::LightInstancerPtr>(protoInstancable);

    // BRAY::ObjectPtr instance
    const BRAY::ObjectPtr    *protoObj =
	BRAY::dyn_cast<BRAY::ObjectPtr>(protoInstancable);

    if (protoLight || protoLightInstancer)
    {
	BRAY::LightInstancerPtr *inst = nullptr;
	{
	    UT_Lock::Scope lock(myLock);
	    // Find existing or create new instance.
	    inst = &myLightInstanceMap[prototypeId];

	    if (!*inst)
	    {
		const UT_StringHolder &name =
		    BRAY_HdUtil::toStr(prototypeId);
		*inst = scene.createLightInstancer(name);
	    }
	}
	...
	else if (protoLightInstancer)
	    inst->addPrototype(scene, *protoLightInstancer);
    }
    else if (protoObj)
    {
	...
	BRAY::ObjectPtr     *inst = nullptr;
	{
		...
		myNewObject = true;	// There's a new object in me
                ...
                if (rparm.needCryptoName() && protoObj->isLeaf() &&
                    BRAY_HdUtil::getUsingStageSceneIndex())
                {
                    // If hydra2 and outputting cryptomatte object layer, get
                    // nice name from scene index
                    //UT_StopWatch timer; timer.start();
                    HdRenderIndex &renderindex =
                        GetDelegate()->GetRenderIndex();
                    HdSceneIndexBaseRefPtr sceneindex =
                        renderindex.GetTerminalSceneIndex();
                    name = BRAY_HdUtil::getAnInstancePath(sceneindex,
                        prototypeId);
                    //UTdebugFormat("{}: {}", prototypeId, timer.stop());
                }
                else
                {
                    // use prototype ID for leaf instances (which will have the
                    // instance ID baked in anyway).  This allows for unique
                    // naming, and matches the non-nested instance naming
                    // convention as well.
                    name =  protoObj->isLeaf()  ?
                            BRAY_HdUtil::toStr(prototypeId) :
                            BRAY_HdUtil::toStr(GetId());
                }
	...
	if (categories)
	{
        ...
        if (!myVisible)
        {
            // Disable per-instance visibility if the instancer itself is set
            // to be invisible
            UT_IntrusivePtr<GT_Int32Array> tmpdata =
                UTmakeIntrusive<GT_Int32Array>(xforms.size(), 1);
            std::fill((int *)tmpdata->getBackingData(),
                (int *)tmpdata->getBackingData() + xforms.size(), 0);

            if (!properties)
                properties = GT_AttributeList::createAttributeList();
            properties =
                properties->addAttribute("visibilitymask", tmpdata, true);
        }
...
BRAY_HdInstancer::eraseFromScenegraph(BRAY::ScenePtr &scene)
{
    ...
    // delete the light collection if there is one
    if (myLightCollection)
	myLightCollection.clear(scene);

    myInstanceMap.clear();
    myLightInstanceMap.clear();
    mySceneGraph = BRAY::ObjectPtr();
    myLightCollection = BRAY::LightInstancerPtr();
}

void
BRAY_HdInstancer::erasePrototype(const SdfPath &protypeid,
    BRAY::ScenePtr &scene)
{
    UT_Lock::Scope lock(myLock);
    UT_Map<SdfPath, BRAY::ObjectPtr>::iterator it =
        myInstanceMap.find(protypeid);
    if (it != myInstanceMap.end())
    {
	UT_ASSERT(it->second);
    ...
    UT_Map<SdfPath, BRAY::LightInstancerPtr>::iterator lit =
        myLightInstanceMap.find(protypeid);
    if (lit != myLightInstanceMap.end())
    {
...
void
BRAY_HdInstancer::Finalize(HdRenderParam *renderParam)
{
    HdInstancer	*parentInstancer =
        GetDelegate()->GetRenderIndex().GetInstancer(GetParentId());
    if (parentInstancer)
    {
        // If nested instancing, erase current instancer from parent instance
        // map
        BRAY_HdParam    *rparm = UTverify_cast<BRAY_HdParam *>(renderParam);
        BRAY::ScenePtr  &scene = rparm->getSceneForEdit();
	BRAY_HdInstancer *instancer = UTverify_cast<BRAY_HdInstancer*>(
...
namespace
{
    ...
    findIndex(const GT_AttributeListHandle &attribs,
            ...
            int tuple_size)
    {
        int     idx = attribs->getIndex(BRAY_HdUtil::usdNameToGT(name));
    ...
    static exint
    primvarSize(const GT_DataArrayHandle &a, const GT_DataArrayHandle &b)
    {
        return SYSmin(a->entries(), b->entries());
    }
        ...
        float shutter_time)
{
            ...
            && myAttributes->get(0)->entries() >= instanceIndices.size())
    {
        ...
        UTisolate([&]()
        {
            ...
            idx = findIndex(myAttributes, instanceTranslationsToken, 3);
            if (idx >= 0)
            {
            ...
            idx = findIndex(myAttributes, instanceRotationsToken, 4);
            if (idx >= 0)
            {
                const auto &data0 = myAttributes->get(idx, seg0);
                const auto &data1 = myAttributes->get(idx, seg1);
            ...
            idx = findIndex(myAttributes, instanceScalesToken, 3);
            if (idx >= 0)
            {
                const auto &data0 = myAttributes->get(idx, seg0);
                const auto &data1 = myAttributes->get(idx, seg1);
            ...
            idx = findIndex(myAttributes, instanceTransformsToken, 16);
            if (idx >= 0)
            {
                const auto &data0 = myAttributes->get(idx, seg0);
                const auto &data1 = myAttributes->get(idx, seg1);

...
namespace
{
    ...
    class bray_HdMeshShareItem : public BRAY_HdParam::ShareItem
    {
    public:
        virtual ~bray_HdMeshShareItem() override {}
        UT_IntrusivePtr<GT_PrimPolygonMesh> myPrim;
        fpreal32                            myCuspAngle;
    };

    static GT_DataArrayHandle
    flipNormals(const GT_DataArrayHandle &oldnmls, const UT_Vector3 *src)
    {
	exint	size = oldnmls->entries();
	auto	nmls = UTmakeIntrusive<GT_Real32Array>(size, 3, GT_TYPE_NORMAL);
        UT_Vector3      *dest = (UT_Vector3 *)nmls->data();
    ...
    static GT_DataArrayHandle
    flipNormals(const GT_DataArrayHandle &oldnmls)
    {
        const void      *data = oldnmls->getBackingData();
        if (data
                && oldnmls->getTupleSize() == 3
                && oldnmls->getStorage() == GT_STORE_REAL32)
        {
            // Fast path
            return flipNormals(oldnmls, (const UT_Vector3 *)data);
        }
	exint	size = oldnmls->entries();
	auto	nmls = UTmakeIntrusive<GT_Real32Array>(size, 3, GT_TYPE_NORMAL);
        UT_Vector3      *dest = (UT_Vector3 *)nmls->data();
...
static void
dumpDirtyBits(const SdfPath &id, const HdDirtyBits &bits)
{
    UTdebugFormat("{} Dirty Bits:", id);
#define DUMP(BIT)       \
    if (bits & HdChangeTracker::BIT) UTdebugFormat("  {}", #BIT);
    DUMP(DirtyPrimID)
    DUMP(DirtyExtent)
    DUMP(DirtyDisplayStyle)
    DUMP(DirtyPoints)
    DUMP(DirtyPrimvar)
    DUMP(DirtyMaterialId)
    DUMP(DirtyTopology)
    DUMP(DirtyTransform)
    DUMP(DirtyVisibility)
    DUMP(DirtyNormals)
    DUMP(DirtyDoubleSided)
    DUMP(DirtyCullStyle)
    DUMP(DirtySubdivTags)
    DUMP(DirtyWidths)
    DUMP(DirtyInstancer)
    DUMP(DirtyInstanceIndex)
    DUMP(DirtyRepr)
    DUMP(DirtyRenderTag)
    DUMP(DirtyComputationPrimvarDesc)
    DUMP(DirtyCategories)
    DUMP(DirtyVolumeField)
#undef DUMP
}
        ...
        TfToken const &repr)
{
	...
	*dirtyBits & HdChangeTracker::DirtyMaterialId)
    ...
    SdfPath		 datashareid;
    const void		*datashareid_key = nullptr;
    int                  datashareid_lid;
    if (UT_EnvControl::getInt(ENV_KARMA_PROTOTYPE_DATA_SHARING))
    {
        ...
        {
            // We're not worried about hash collisions since if it actually
            // ever happens, we'll just be doing an extra lock (which doesn't
            // break anything).
            datashareid_key = reinterpret_cast<const void *>(datashareid.GetHash());
        }
    }
    ...
    switch (GetCullStyle(sceneDelegate))
    {
        case HdCullStyleDontCare:
            break;
        case HdCullStyleNothing:
            props.set(BRAY_OBJ_CULL_BACKFACE, 0);
            break;
        case HdCullStyleBack:
            props.set(BRAY_OBJ_CULL_BACKFACE, 1);
            break;
        case HdCullStyleFront:
            props.set(BRAY_OBJ_CULL_BACKFACE, 2);
            break;
    ...
    props_changed |=  (prevbface != *props.ival(BRAY_OBJ_CULL_BACKFACE));
	...
	if ((poly_holes && pmesh->getFaceCount() != top.GetFaceVertexCounts().size())
	    ...
	    || !BRAY_HdUtil::matchAttributes(sceneDelegate, rparm, id, primType,
		    thePtInterp, SYSarraySize(thePtInterp), pmesh->getShared(),
                    mptrs, skipN)
	    || !BRAY_HdUtil::matchAttributes(sceneDelegate, rparm, id, primType,
		    HdInterpolationFaceVarying, pmesh->getVertex(),
                    mptrs, skipN))
	{
	    top_dirty = true;
            props_changed = true;
	...
	if (myMesh)
	    prim = UTverify_cast<GT_PrimPolygonMesh*>(myMesh.geometry().get());
	...
	if (!(event & (BRAY_EVENT_ATTRIB|BRAY_EVENT_ATTRIB_P)))
	{
	    ...
	    alist[0] = prim->getVertex();
	    alist[1] = prim->getShared();
	    alist[2] = prim->getUniform();
	    alist[3] = prim->getDetail();
	}
	if (!counts)
        {
	    UT_ASSERT(prim);
	    counts = prim->getFaceCounts();
        }
	if (!vlist)
        {
	    UT_ASSERT(prim);
	    vlist = prim->getVertexList();
        }
		...
		&& gtScheme(scheme) != GT_SCHEME_INVALID)
	{
	    ...
	    auto subd = UTmakeIntrusive<GT_PrimSubdivisionMesh>(counts, vlist,
		    alist[1],	// Shared
		    alist[0],	// Vertex
		    alist[2],	// Uniform
		    alist[3]);	// detail
	...
	else
	{
		...
		prim = UTmakeIntrusive<GT_PrimPolygonMesh>(
		...
		prim = UTmakeIntrusive<GT_PrimPolygonMesh>(counts, vlist,
			alist[1],	// Shared
			alist[0],	// Vertex
			alist[2],	// Uniform
			alist[3]);	// detail
		...
		if (!hasNormals(*prim))
		{
                    ...
                    //(GEO_DEFAULT_ADJUSTED_CUSP_ANGLE/GEO_DEFAULT_CUSP_ANGLE)
                    cuspangle *= 1.001;

                    auto computeNormals = [&]()
                    {
                        UT_IntrusivePtr<GT_PrimPolygonMesh> newmesh;
                        bool vertexnormals;
                        if (scheme == PxOsdOpenSubdivTokens->bilinear ||
                            scheme == PxOsdOpenSubdivTokens->none)
                        {
                            vertexnormals = true;
                            newmesh = prim->createVertexNormalsIfMissing(
                                            GA_Names::P,
                                            cuspangle);
                        }
                        else
                        {
                            ...
                            // displacement.
                            vertexnormals = false;
                            newmesh = prim->createPointNormalsIfMissing(
                                            GA_Names::P);
                        }

                        if (newmesh && newmesh != prim)
                        {
                            if (!myLeftHanded)
                            {
                                // Vertex normals are computed with the
                                // assumption that pmesh is left-handed, so
                                // must be flipped
                    ...
                    if (!datashareid.IsEmpty())
                    {
                        ...
                        // (hold onto the accessor until normals are computed
                        // to block other threads with the same shared data id
                        // from computing normals and eat up memory before
                        // they're deduplicated by karma)
                        {
                            BRAY_HdParam::DataSharingCache::accessor found;
                            if (rparm.insertDataShareItem(found, datashareid))
                            {
                            ...
                            bray_HdMeshShareItem *item =
                                UTverify_cast<bray_HdMeshShareItem*>(
                                found->second.get());
                            if (item && item->myCuspAngle == cuspangle &&
                                item->myPrim)
                            {
                                // check motion segments (in case they had
                                // differing vblur/nseg properties despite
                                // having the same data sharing id)
                                const GT_AttributeListHandle &attrsrc =
                                    item->myPrim->getShared();
                                const GT_AttributeListHandle &attrdst =
                                    prim->getShared();
                    ...
                    // Did we make normals successfully:
                    myComputeN = hasNormals(*prim);
		}
	    }
	}

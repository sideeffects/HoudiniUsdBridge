	...
	TfToken const &repr)
{
	...
	if (!BRAY_HdUtil::matchAttributes(sd, *rparm, id, primType,
		    HdInterpolationConstant, pmesh->getUniform(),
                    mptrs)
	    || !BRAY_HdUtil::matchAttributes(sd, *rparm, id, primType,
		    thePtInterp, SYSarraySize(thePtInterp), pmesh->getPoints(),
                    mptrs, &theSkipIds)
            || (P && P->getTupleSize() != 3))
	{
	    topo_dirty = true;
	...
	event = (event | BRAY_EVENT_TOPOLOGY
        ...
        // For a procedural, we need to include all attributes, not just those
        // used by shading.
        if (!BRAY_HdUtil::isKarmaProcedural(sd, id))
            BRAY_HdUtil::getRequiredPrimvars(mptrs, scene, *rparm, matId);
	...
	alist[1] = BRAY_HdUtil::makeAttributes(sd, *rparm, id,
		primType, 1, props, HdInterpolationConstant, mptrs);
    ...
    for (auto &&p : myPrims)
    {
        p.setCoordSysAliases(scene,
                BRAY_HdUtil::getCoordSysBindings(sd, id));
    }

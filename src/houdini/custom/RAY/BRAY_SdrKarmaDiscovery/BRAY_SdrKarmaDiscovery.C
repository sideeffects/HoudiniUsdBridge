...
SDR_REGISTER_DISCOVERY_PLUGIN(BRAY_SdrKarmaDiscovery)
...
static SYS_FORCE_INLINE size_t
UTformatBuffer(char *buffer, size_t bufsize, const SdrTokenVec &v)
{
    UT_WorkBuffer       tmp;
    if (v.size())
    {
...
{
    TF_DEBUG(SDR_DISCOVERY).Msg("SdrKarmaDiscovery c-tor");
}
...
static void
makeShaderNode(SdrShaderNodeDiscoveryResultVec &nodes, const KarmaNode &node)
{
...
static void
makeShaderNode(SdrShaderNodeDiscoveryResultVec &nodes, const UT_StringRef &name_ref)
{
...
BRAY_SdrKarmaDiscovery::DiscoverShaderNodes(const Context &)
{

...
GEO_AgentChannelEvaluator::GEO_AgentChannelEvaluator()
    : UT_IntrusiveRefCounter<GEO_AgentChannelEvaluator>()
    , myEvaluateTask(*this)
{
}

void
GEO_AgentChannelEvaluator::EvaluateTask::operator()()
{
    UT_StringHolder error_msg;
    myOwner.myIsValid = myOwner.myEvaluator.evaluate(
            error_msg);

    if (!myOwner.myIsValid)
        TF_WARN("Error evaluating ML deformer: %s", error_msg.buffer());
}

exint
GEO_AgentChannelEvaluator::addAgent(const GU_Agent *agent)
{
    UT_ASSERT(!myExclusive.hasRun());

    return myEvaluator.addAgent(agent);
}

UT_Span<const GU_Agent::FloatType>
GEO_AgentChannelEvaluator::getChannelValues(
        ...
        exint id) const
{
    ...
    myExclusive.execute(myEvaluateTask);

    if (!myIsValid)
        return {};

    return myEvaluator.getChannelValues(agent, id);
}
...
GT_PrimSkelAnimation::GT_PrimSkelAnimation(
    ...
    : myAgent(agent)
    , mySkelPrim(skel)
    , myChannelEvaluator(channel_evaluator)
    , myChannelEvaluatorId(channel_evaluator->addAgent(agent))
{
}

bool
GT_PrimSkelAnimation::computeChannelValues(VtFloatArray &weights) const
{
    weights.clear();

    // Use the channel evaluator if necessary (e.g. ML deformer is present)
    if (myChannelEvaluatorId >= 0)
    {
        UT_Span<const GU_Agent::FloatType> channel_values
                = myChannelEvaluator->getChannelValues(
                        myAgent, myChannelEvaluatorId);
        if (!channel_values.empty())
        {
            weights = VtFloatArray(
                    channel_values.begin(), channel_values.end());
            return true;
        }
    }

    // Otherwise, just evaluate the agent's regular channel values.
    GU_Agent::FloatArrayConstPtr channel_values;
    if (myAgent->computeChannelValues(channel_values))
    {
        weights = VtFloatArray(channel_values->begin(), channel_values->end());
        return true;
    }

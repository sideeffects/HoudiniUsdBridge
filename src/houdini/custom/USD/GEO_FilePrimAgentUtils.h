...
#include <GU/GU_AgentMLSkinDeformer.h>
...
class GEO_AgentChannelEvaluator;
using GEO_AgentChannelEvaluatorHandle
        = UT_IntrusivePtr<GEO_AgentChannelEvaluator>;
...
/// Batch evaluator for agents which require further work to compute their
/// exported blendshape weights (namely, evaluating the ML deformer).
class GEO_AgentChannelEvaluator
    : public UT_IntrusiveRefCounter<GEO_AgentChannelEvaluator>
{
public:
    GEO_AgentChannelEvaluator();

    /// Returns a unique ID for retrieving the agent's channel values if
    /// additional evaluation is required. Otherwise, returns -1.
    exint addAgent(const GU_Agent *agent);

    /// Returns the rig channel values for the specified agent.
    UT_Span<const GU_Agent::FloatType> getChannelValues(
...
private:
    ...
    {
    public:
        explicit EvaluateTask(GEO_AgentChannelEvaluator &owner)
            : myOwner(owner)
        {
        }
    ...
    private:
        GEO_AgentChannelEvaluator &myOwner;
    };

    mutable GU_AgentBakeMLSkinDeformer myEvaluator;
    mutable bool myIsValid = false;
    /// Lazily evaluate the blendshape weights after all agents have been
    /// registered.
    mutable EvaluateTask myEvaluateTask;
    mutable UT_TaskExclusive<EvaluateTask> myExclusive;
};
...
{
public:
    GT_PrimSkelAnimation(
            const GU_Agent *agent,
            const GT_PrimSkeletonPtr &skel,
            const GEO_AgentChannelEvaluatorHandle &channel_evaluator);
    ...
    /// Returns the blend shape channel values. This may require additional
    /// computation beyond GU_Agent::computeChannelValues() (e.g. baking the ML
    /// deformer).
    bool computeChannelValues(VtFloatArray &weights) const;
...
private:
    ...
    GEO_PathHandle myPath;
    GEO_AgentChannelEvaluatorHandle myChannelEvaluator;
    exint myChannelEvaluatorId = -1;
};

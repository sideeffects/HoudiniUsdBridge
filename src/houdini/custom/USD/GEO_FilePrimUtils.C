...
/// For indexed attributes, allow using `type:string` to select any string
/// attribute.
static inline bool
geoMultiMatchIndexedAttrib(
        const UT_StringMMPattern &pattern,
        const UT_StringRef &attr_name,
        const UT_StringRef &decoded_attr_name,
        const GT_Storage storage)
{
    ...
    // Evaluate the pattern against both "type:string" and the attrib name, and
    // use the later match as the result. We also need to enable explicit
    // exclusions so that a pattern such as "type:string ^bar" records that
    // "bar" was removed without a prior positive match.
    ...
    const bool matched_type = theStringTypeName.asRef().multiMatch(

...
TF_REGISTRY_FUNCTION_WITH_TAG(TfType, USD_HD_DrawModeAdapter)
{
    typedef HD_DrawModeAdapter Adapter;
...
bool
HD_DrawModeAdapter::ShouldCullChildren() const
{
    return true;
}

bool
HD_DrawModeAdapter::CanPopulateUsdInstance() const
{
    return true;
}

bool
HD_DrawModeAdapter::IsSupported(
...
SdfPath
HD_DrawModeAdapter::Populate(UsdPrim const& prim,
                                       ...
                                       instancerContext)
{
    ...
    if (drawMode == UsdGeomTokens->origin) {
        ...
        else if (index->IsRprimTypeSupported(HdPrimTypeTokens->basisCurves)) {
            // cache the draw mode before inserting the prim
            _drawModeMap.insert({ cachePath, drawMode });
...
void
HD_DrawModeAdapter::_RemovePrim(SdfPath const& cachePath,
                                       UsdImagingIndexProxy* index)
{
...
void
HD_DrawModeAdapter::MarkDirty(UsdPrim const& prim,
...
void
HD_DrawModeAdapter::MarkTransformDirty(UsdPrim const& prim,
                                              SdfPath const& cachePath,
                                              UsdImagingIndexProxy* index)
{
    if (!_IsMaterialPath(cachePath)) {
...
void
HD_DrawModeAdapter::MarkVisibilityDirty(UsdPrim const& prim,
                                               SdfPath const& cachePath,
                                               UsdImagingIndexProxy* index)
{
    if (!_IsMaterialPath(cachePath)) {
...
void
HD_DrawModeAdapter::MarkMaterialDirty(UsdPrim const& prim,
                                               SdfPath const& cachePath,
                                               UsdImagingIndexProxy* index)
{
    if (_IsMaterialPath(cachePath)) {
...
bool
HD_DrawModeAdapter::_HasVaryingExtent(UsdPrim const& prim) const
{
    UsdAttribute attr;
...
void
HD_DrawModeAdapter::_ComputeGeometryData(
    UsdPrim const& prim,
    SdfPath const& cachePath,
    UsdTimeCode time,
...
/*virtual*/ 
VtValue
HD_DrawModeAdapter::GetTopology(UsdPrim const& prim,
                                         SdfPath const& cachePath,
                                 ...
                                 VtIntArray *outIndices) const
{
    ...
    } else if (key == HusdHdPrimvarTokens->glWire) {
        int wireframe = 1;
        value = wireframe;
...
void
HD_DrawModeAdapter::_CheckForTextureVariability(
...
void
HD_DrawModeAdapter::TrackVariability(UsdPrim const& prim,
                                            SdfPath const& cachePath,
...
void
HD_DrawModeAdapter::UpdateForTime(UsdPrim const& prim,
                                         SdfPath const& cachePath,
                                         UsdTimeCode time,
                                            ...
                                            instancerContext) const
{
    ...
    if (requestedBits & HdChangeTracker::DirtyPrimvar) {
        ...
        _MergePrimvar(&primvars, HdTokens->displayOpacity,
                      HdInterpolationConstant);
        _MergePrimvar(&primvars, HusdHdPrimvarTokens->glWire,
                      HdInterpolationConstant);
    }
...
void
HD_DrawModeAdapter::_GenerateBoundsCurveGeometry(
        VtValue *topo, VtValue *points, GfRange3d const& extents) const
{
...
void
HD_DrawModeAdapter::_SanityCheckFaceSizes(SdfPath const& cachePath,
...
bool
HD_DrawModeAdapter::_GetMatrixFromImageMetadata(
...
/*virtual*/
HdCullStyle 
HD_DrawModeAdapter::GetCullStyle(UsdPrim const& prim,
                                          SdfPath const& cachePath,
                                          UsdTimeCode time) const
{

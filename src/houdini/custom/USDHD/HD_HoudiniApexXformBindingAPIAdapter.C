...
#include "HD_HoudiniApexXformBindingAPIAdapter.h"

#include <HUSD/UsdHoudini/houdiniApexXformBindingAPI.h>
...
TF_REGISTRY_FUNCTION_WITH_TAG(
        TfType,
        USD_HD_HoudiniApexXformBindingAPIAdapter)
{
    using Adapter = HD_HoudiniApexXformBindingAPIAdapter;
...
namespace
{
/// Map the HoudiniApexXformBindingAPI attributes to Hydra data sources.
        ...
        {
            {
                UsdSchemaRegistry::MakeMultipleApplyNameInstance(
                    ...
                    instance_name),
                HdDataSourceLocator(HusdHdApexTokens->joint)
            }
        },
...
/// Data source for the list of xform bindings.
/// This has one child container named after the API schema's instance name,
/// e.g. 'myprop', which in turn contains the schema's attributes.
...
private:
    UsdHoudiniHoudiniApexXformBindingAPI myAPI;
...
HD_HoudiniApexXformBindingAPIAdapter::HD_HoudiniApexXformBindingAPIAdapter() = default;

HD_HoudiniApexXformBindingAPIAdapter::~HD_HoudiniApexXformBindingAPIAdapter() = default;

HdContainerDataSourceHandle
HD_HoudiniApexXformBindingAPIAdapter::GetImagingSubprimData(
        UsdPrim const& prim,
        ...
        const UsdImagingDataSourceStageGlobals& stage_globals)
{
    ...
    // e.g. 'houdiniApexXformBindings/myprop'
...
HD_HoudiniApexXformBindingAPIAdapter::InvalidateImagingSubprim(
        UsdPrim const& prim,
        TfToken const& subprim,
        TfToken const& applied_instance_name,
        ...
        UsdImagingPropertyInvalidationType invalidation_type)
{
    ...
    // instance of the API schema. e.g 'houdinig:apex:xform:myprop:*'
    std::string prefix = TfStringPrintf(
            "%s:%s:", UsdHoudiniTokens->houdiniApexXform.data(),

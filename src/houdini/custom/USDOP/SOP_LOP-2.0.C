...
static const char* theDsFile = R"THEDSFILE(
{
    ...
    parm {
        name    "excludeinactiveprims"
        cppname "ExcludeInactivePrims"
...
void
SOP_LOP2::syncNodeVersion(
        const char *old_version,
        const char *cur_version,
        bool *node_deleted)
{
    // Before 21.0.184, inactive prims were not excluded by default.
    if (UT_String::compareVersionString(old_version, "21.0.184") < 0)
    {
        setInt("excludeinactiveprims"_sh, 0, 0.0, 0);
    }
}
...
public:
    ...
    void update(const OP_Context &context)
    {
        ...
        myLOPMicroNode.updateOptions(context);
...
SOP_LOP2Verb::cook(const CookParms &cookparms) const
{
        ...
        || cache.myTopologyId != gdp->getTopology().getDataId())
    {
        ...
        if (cache.requiresStageUpdate(context, parms))
        {
            ...
            ev_GlobalEvalLock().lockedExecute([&]() {
                ...
                cache.myLockedStage = HUSD_LockedStageRegistry::getInstance().
                    getLockedStage(lop, 0, cache.myDataHandle, cache.myStripLayers,
        ...
        if (cache.myExcludeInactivePrims)
            pattern.append(" & %active:true");

...
public:
    ...
    DEP_TimedMicroNode myLOPMicroNode;
...
SOP_LOP2Verb::cook(const CookParms &cookparms) const
{
    ...
    const HUSD_TimeCode timecode(parms.getImportFrame(), HUSD_TimeCode::FRAME);
    ...
    if (cache.myLOPMicroNode.requiresUpdate(cookparms.getCookTime())
        ...
        || cache.myTopologyId != gdp->getTopology().getDataId())
    {
        ...
        context.setFrame(parms.getImportFrame());
        ...
        HUSD_LockedStagePtr locked_stage
                = HUSD_LockedStageRegistry::getInstance().getLockedStage(
                        lop->getUniqueId(), data_handle, cache.myStripLayers,
        ...
        if (!findprims.addPattern(cache.myPrimPattern, lop->getUniqueId(), timecode))
        {
            cookparms.addError(
        ...
        purposes.SetConstant(GusdPurposeSet(
                GusdPurposeSetFromMask(cache.myPurpose)
        ...
        // cook.
        cache.myLOPMicroNode.update(cookparms.getCookTime());
    }
    else
    {

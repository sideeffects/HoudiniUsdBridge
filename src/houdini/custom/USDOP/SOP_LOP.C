...
SOP_LOP::_CreateNewPrims(OP_Context& ctx, const GusdUSD_Traverse* traverse)
{
	...
	getLockedStage(lop, 0, datahandle,
	    strip_layers, lopctx.getTime(),
            HUSD_IGNORE_STRIPPED_LAYERS);

    HUSD_AutoReadLock	 readlock(datahandle);
    HUSD_FindPrims	 findprims(readlock, HUSD_TRAVERSAL_DEFAULT_WITH_PROXIES);
    GusdStageCacheReader cache;
    UsdStageRefPtr	 stage = cache.Find(locked_stage->
                            getStageCacheIdentifier().toStdString());

...
#ifdef KARMA_OUTPUT_GPU_BUFFERS
...
#define HOM_AUTO_UNLOCK_ON_MAIN_THREAD() \
    UT_UniquePtr<HOM_AutoUnlock> hom_auto_unlock; \
    if (UT_Thread::isMainThread() && HOMgetLock().hasLock()) { \
	hom_auto_unlock = UTmakeUnique<HOM_AutoUnlock>(); }
...
XUSD_ImagingEngineGL::XUSD_ImagingEngineGL(const Parameters &params)
    : XUSD_ImagingEngine(params)
    , _domeLightCameraVisibility(true)
    , _displayUnloaded(params.displayUnloadedPrimsWithBounds)
        ...
        const UsdImagingGLRenderParams& params)
{
    ...
    enableMaterialsLights(params.enableSceneMaterials, params.enableSceneLights);
}
...
XUSD_ImagingEngineGL::_UpdateDomeLightCameraVisibility()
{
    ...
    if (_domeLightCameraVisibility != domeLightCamVisSetting) {
        ...
        SetDomeLightCameraVisibility(domeLightCamVisSetting);
    }
}
        ...
        const XUSD_ImagingRenderParams &params)
{
    ...
    // Unlock the HOM lock if we have it because syncing the renderer
    // may require stopping the render, which in turn may require
...
#ifdef KARMA_OUTPUT_GPU_BUFFERS
        else if (resource.CanCast<BRAY::AOVBufferPtr>())
        {
            BRAY::AOVBufferPtr aovbuf = resource.UncheckedGet<BRAY::AOVBufferPtr>();
            if (aovbuf.getFormat() == PXL_INT32)
            {
                auto it = _aovRasters.find(aovbuf.getName());
                if (it != _aovRasters.end())
                {
                    delete it->second;
                    _aovRasters.erase(it);
                }
                PXL_Raster raster(
                        aovbuf.getPacking(),
                        aovbuf.getFormat(),
                        aovbuf.getXres(),
                        aovbuf.getYres());
                raster.setRaster(aovbuf.map(), false);
                PXL_Raster *rastercopy = new PXL_Raster(raster);
                aovbuf.unmap();
                REgetRender()->convertMaskToTexture(rastercopy);
                if (rastercopy->getTextureID())
                {
                    _aovRasters[aovbuf.getName()] = rastercopy;
                    id = ((RE_OGLTexture*)rastercopy->getTextureID())->getID();
                    width = aovbuf.getXres();
                    height = aovbuf.getYres();
                    int *pixels = (int *)rastercopy->getPixels();
...
#endif // KARMA_OUTPUT_GPU_BUFFERS
    }
    ...
    GfVec4f const &sceneAmbient)
{
    ...
    const char *defaultDomeLightTex =
    ...
    std::vector<HdInstancerContext> &outHitInstancerContexts)
{
    ...
    {
        outHitPrimPaths.resize(inOutKeys.size());
        outHitInstancerContexts.resize(inOutKeys.size());
        UTparallelFor(UT_BlockedRange<int>(0, inOutKeys.size()),
            [this,
             &inOutKeys,
             &outHitPrimPaths,
             &outHitInstancerContexts](const UT_BlockedRange<int> &r)
            {
                SdfPath delegateId, instancerId, primPath;
                std::vector<HdxPickHit> hits;

                hits.resize(r.size());
                for (int keyidx = 0; keyidx < r.size(); keyidx++)
                {
                    primPath = _renderIndex->GetRprimPathFromPrimId(
                        inOutKeys[keyidx + r.begin()].myPickId);
                    _renderIndex->GetSceneDelegateAndInstancerIds(
                        primPath, &delegateId, &instancerId);

                    HdxPickHit &hit = hits[keyidx];
                    hit.delegateId = delegateId;
                    hit.objectId = primPath;
                    hit.instancerId = instancerId;
                    hit.instanceIndex = inOutKeys[keyidx + r.begin()].myInstId;
                }

                const std::vector<HdxPrimOriginInfo> infos =
                    HdxPrimOriginInfo::FromPickHits(_renderIndex.get(), hits);

                for (int keyidx = 0; keyidx < infos.size(); keyidx++)
                {
                    const HdxPrimOriginInfo &info = infos[keyidx];
                    primPath = info.GetFullPath();
                    instancerId = instancerId.ReplacePrefix(_sceneDelegateId,
                        SdfPath::AbsoluteRootPath());
                    outHitPrimPaths[keyidx + r.begin()] = primPath;
                    outHitInstancerContexts[keyidx + r.begin()] =
                        info.ComputeInstancerContext();
                }
            }, 1 /* oversubscribe ratio */,
            4096 /* min_grain_size */,
            false /* task_scope */);
    }
    else
        ...
        for (auto it : instancerMap)
        {
            ...
            if (primPaths.size() > 0)
            {
                ...
                if (instancerContexts.size() > 0)
                    outHitInstancerContexts.insert(
                        outHitInstancerContexts.end(),
                        instancerContexts.begin(),
                        instancerContexts.end());
                else
                    outHitInstancerContexts.insert(
                        outHitInstancerContexts.end(),
                        primPaths.size(),
        ...
        const HdRenderSettingsMap &settings)
{
    ...
    if (_HasRenderer() && _renderDelegate.GetPluginId() == resolvedId) {
        return true;
    }
...
void
XUSD_ImagingEngineGL::DestroyHydraResources()
{
    _DestroyHydraObjects();
}
    ...
    HdPluginRenderDelegateUniqueHandle &&renderDelegate)
{
    ...
    std::string renderInstanceId = createAppSceneIndices(renderDelegate);
    ...
    if (!_cameraPath.IsEmpty())
        _taskController->SetCameraPath(_cameraPath);

    // Create the new scene API (including the _engine)
    createSceneAPI(_displayUnloaded, _enableUsdDrawModes);
}
...
XUSD_ImagingEngineGL::SetRendererAovs(TfTokenVector const &ids)
{
    ...
    if (_renderIndex->IsBprimTypeSupported(HdPrimTypeTokens->renderBuffer))
    {
        ...
        if (myFastPathColor && ids.size() == 1 && ids[0] == HdAovTokens->color) {
...
{
    // Unlock the HOM lock if we have it because changing a render setting
    // may require stopping the renderer, which in turn may require
    // completing some number of operations (reading textures)
    // that require locking the HOM lock (cooking COP nodes).
    HOM_AUTO_UNLOCK_ON_MAIN_THREAD();
...
{
    // Unlock the HOM lock if we have it because pausing the renderer
    // may require completing some number of operations (reading textures)
    // that require locking the HOM lock (cooking COP nodes).
    HOM_AUTO_UNLOCK_ON_MAIN_THREAD();
...
{
    // Unlock the HOM lock if we have it because stopping the renderer
    // may require completing some number of operations (reading textures)
    // that require locking the HOM lock (cooking COP nodes).
    HOM_AUTO_UNLOCK_ON_MAIN_THREAD();
        ...
        const UT_StringHolder &command_name) const
{
    if (_HasRenderer())
    {
        // Unlock the HOM lock if we have it because invoking a render command
        // may require stopping the renderer, which in turn may require
        // completing some number of operations (reading textures)
        // that require locking the HOM lock (cooking COP nodes).
        HOM_AUTO_UNLOCK_ON_MAIN_THREAD();
...
bool
XUSD_ImagingEngineGL::PollForAsynchronousUpdates() const
{

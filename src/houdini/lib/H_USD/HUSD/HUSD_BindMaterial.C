...
static const std::vector<TfToken> thePurposes{
...
HUSD_BindMaterial::HUSD_BindMaterial( HUSD_AutoWriteLock &lock )
    ...
    , myBindCollectionWarnUnspecifiedPrim( true )
    , myStrength( Strength::DEFAULT )
    , myPurpose( HUSD_Constants::getMatPurposeAll() )
    , myBindCollectionPrimPath("/geo")
{
}
...
husdGetBindPrim(HUSD_AutoAnyLock &lock,
        const UT_StringRef &path,
	const HUSD_FindPrims *find_prims,
        const UT_StringRef *path_expr,
        bool add_warning_if_not_specified)
{
    UT_StringHolder final_path( path );
    bool add_warning = add_warning_if_not_specified;

    if (!final_path.isstring() && find_prims)
    {
	final_path = find_prims->getSharedRootPrim();
    }
    else if (!final_path.isstring() && path_expr)
    {
        HUSD_FindPrims path_expr_find_prims(lock);
        path_expr_find_prims.addPathExpression(*path_expr);
        final_path = path_expr_find_prims.getSharedRootPrim();
    }
    else
    {
        // Binding primitive path was supplied, so no need to warn.
        add_warning = false;
    }
...
{
    UT_StringHolder path = include_geo_prims.getSingleCollectionPath();
...
static inline UsdCollectionAPI
husdGetBindCollectionWithPathExpression(
        UsdStageRefPtr &stage, HUSD_AutoWriteLock &lock,
        const UT_StringRef &path_expr,
        UsdPrim &prim, const UT_StringRef &collection_name,
        bool collection_expand_prims )
{
    ...
    if( !col_creator.createPathExpressionCollection(
            prim.GetPath().GetString(), collection_name,
            path_expr, false ))
    {
        return UsdCollectionAPI();
    }
...
static inline void
husdSetGeoSubsetFamilyName(UsdPrim prim)
{
    UsdGeomSubset geo_subset(prim);
    if( geo_subset )
    {
        auto family_name_attr = geo_subset.GetFamilyNameAttr();
        if (!family_name_attr)
            family_name_attr = geo_subset.CreateFamilyNameAttr();
        if (family_name_attr)
        {
            TfToken family_name;

            // Set the family name on the subset to materialBind if it
            // isn't already set.
            if (!family_name_attr.Get(&family_name) ||
                family_name != UsdShadeTokens->materialBind)
            {
                UsdGeomImageable parent(prim.GetParent());

                family_name_attr.Set(UsdShadeTokens->materialBind);
                // If we have to set the family name on the subset, we may
                // also need to set the family type on the parent imageable.
                if (parent)
                {
                    TfToken family_type = geo_subset.GetFamilyType(parent,
                        UsdShadeTokens->materialBind);

                    if (family_type != UsdGeomTokens->partition &&
                        family_type != UsdGeomTokens->nonOverlapping)
                        geo_subset.SetFamilyType(parent,
                            UsdShadeTokens->materialBind,
	...
	HUSD_BindMaterial::Strength strength, const UT_StringRef &purpose) 
{
    ...
    husdSetGeoSubsetFamilyName(prim);
	...
	const HUSD_FindPrims &find_geo_prims) const
{
    ...
    {
	auto binding_prim = husdGetBindPrim(myWriteLock,
            myBindCollectionPrimPath, &find_geo_prims, nullptr,
            myBindCollectionWarnUnspecifiedPrim);
	...
	UT_StringHolder	collection_name( myBindCollectionName );
	if( !collection_name )
	    collection_name = husdMakeValidSdfPath(mat_prim_path).GetName();

	auto collection = husdGetBindCollection( stage, myWriteLock,
		find_geo_prims, HUSD_FindPrims(myWriteLock),
                binding_prim, collection_name,
		myBindCollectionExpand );

        for (auto &&primpath : find_geo_prims.getExpandedPathSet())
            husdSetGeoSubsetFamilyName(
                stage->GetPrimAtPath(primpath.sdfPath()));

        UT_String binding_name(myBindCollectionName.c_str());
        if( !binding_name.isstring() )
            binding_name = collection.GetName();
        HUSDmakeValidUsdName(binding_name, false);

	return husdBindCollection(stage, mat_prim_path, collection,
            binding_prim, TfToken(binding_name.toStdString()),
            myStrength, myPurpose);
    }
...
bool
HUSD_BindMaterial::bindWithCollection(const UT_StringRef &mat_prim_path,
        const HUSD_FindPrims &include_geo_prims,
        const HUSD_FindPrims &exclude_geo_prims) const
{
    ...
    // Only collection based binding is supported.
    if( myBindMethod == BindMethod::COLLECTION )
    {
        auto binding_prim = husdGetBindPrim(myWriteLock,
            myBindCollectionPrimPath, &include_geo_prims, nullptr,
            myBindCollectionWarnUnspecifiedPrim);
        if( !binding_prim || binding_prim.IsPseudoRoot() )
        {
            HUSD_ErrorScope::addError( HUSD_ERR_STRING,
        ...
        auto collection = husdGetBindCollection(stage, myWriteLock,
            include_geo_prims, exclude_geo_prims,
            binding_prim, collection_name,
            myBindCollectionExpand);

        for (auto &&primpath : include_geo_prims.getExpandedPathSet())
            husdSetGeoSubsetFamilyName(
                stage->GetPrimAtPath(primpath.sdfPath()));
    ...
    UT_ASSERT( !"Unsupported binding method requested" );
    return false;
}

bool
HUSD_BindMaterial::bindWithPathExpression(const UT_StringRef &mat_prim_path,
        const UT_StringRef &path_expr) const
{
    ...
    {
        auto binding_prim = husdGetBindPrim(myWriteLock,
            myBindCollectionPrimPath, nullptr, &path_expr,
            myBindCollectionWarnUnspecifiedPrim);
        if( !binding_prim || binding_prim.IsPseudoRoot() )
        {
            HUSD_ErrorScope::addError( HUSD_ERR_STRING,
        ...
        auto collection = husdGetBindCollectionWithPathExpression(
            stage, myWriteLock,
            path_expr, binding_prim, collection_name,
            myBindCollectionExpand );
	...
	const UsdTimeCode &tc)
{
    ...
    if( geo_subset )
    {
        husdSetGeoSubsetFamilyName(geo_subset.GetPrim());
	...
	const HUSD_TimeCode &tc /*=HUSD_TimeCode()*/) const
{
    ...
    UsdGeomMesh geo = UsdGeomMesh::Get(stage, HUSDgetSdfPath( geo_prim_path ));
    if( !geo )
    {
        HUSD_ErrorScope::addWarning(HUSD_ERR_SUBSETS_ONLY_ON_MESH_PRIMITIVES,
		geo_prim_path.c_str());
...
{
    if( rel.GetPrim().IsInstanceProxy() )
    {
        HUSD_ErrorScope::addWarning(
                HUSD_ERR_IGNORING_INSTANCE_PROXY,
                rel.GetPrimPath().GetAsString().c_str());
    }
    else if( !rel.SetTargets({}) )
    {
	UT_WorkBuffer msg;
...
bool
HUSD_BindMaterial::isPrimWithoutBoundMaterial( const UT_StringRef &prim_path,
        const UT_Optional<UT_StringHolder> &purpose )
{
    UsdStageRefPtr stage = myWriteLock.data()->stage();
    if( !stage || !HUSDisValidUsdPath( prim_path ))
        return false;

    UsdPrim prim = stage->GetPrimAtPath( HUSDgetSdfPath( prim_path ));
    if( !prim )
        return false;

    // Decide which purposes to check.
    std::vector<TfToken> purposes;
    if( purpose )
        purposes.emplace_back( purpose->toStdString() );
    else
        purposes = thePurposes;

    UsdShadeMaterialBindingAPI bindingapi(prim);
    for( auto &purpose : purposes )
        if( bindingapi.ComputeBoundMaterial( purpose ))
            return false;

    return true; // prim without a material binding
}
...
HUSD_BindMaterial::unbindAll( const HUSD_FindPrims &find_prims ) const
{
    ...
    bool ok = true;
    for( auto const & purpose: thePurposes )
...
{
    UsdShadeMaterialBindingAPI::BindingsCache cache;
    ...
    {
        SdfLayer::FileFormatArguments sdfargs;
    ...
    {
        layer = myWriteLock.data()->activeLayer();
        edit_source_layer = true;
    }
    stage = myWriteLock.data()->stage();

    if (!layer || !stage)
    {
        HUSD_ErrorScope::addError(HUSD_ERR_CANT_FIND_LAYER, layername.c_str());
        return true;
    }

    TfToken attrnametoken(attrname.toStdString());
    TfToken reffileattrnametoken(reffileattrname.toStdString());
    TfToken refprimattrnametoken(refprimattrname.toStdString());
    SdfPathSet paths;
    ...
    layer->Traverse(SdfPath::AbsoluteRootPath(),
        [&](const SdfPath &path)
        {
            if (path.IsPropertyPath() &&
                attrnametoken == path.GetName())
                paths.insert(path);
        });

    for (auto &&path : paths)
    {
        auto attrspec = layer->GetPropertyAtPath(path);
        if (!attrspec)
            continue;

        auto reffilespec = layer->GetPropertyAtPath(
                path.ReplaceName(reffileattrnametoken));
        auto refprimspec = layer->GetPropertyAtPath(
                path.ReplaceName(refprimattrnametoken));

        SdfPath geopath = sdfrefprimpath.IsEmpty()
                ? path.GetPrimPath()
                : path.GetPrimPath().ReplacePrefix(
                    sdfrefprimpath, sdfprimpath);
        UsdPrim geoprim = stage->GetPrimAtPath(geopath);

        if (!geoprim)
            continue;

        auto targetvalue = attrspec->GetDefaultValue();
        auto targetstr = targetvalue.Get<std::string>();
        ...
        if (!targetstr.empty())
        {
            ...
            if (!targetprim && create_empty_materials)
            {
                ...
                std::string refprim;
                if (refprimspec)
                    refprim = refprimspec->GetDefaultValue().Get<std::string>();
                ...
                else
                {
                    ...
                    createprims.createPrim(targetpath.GetAsString(),
                        HUSD_Constants::getMaterialPrimTypeName(),
                        UT_StringHolder::theEmptyString,
                        HUSD_Constants::getPrimSpecifierDefine(),
                        HUSD_Constants::getScopePrimType());
                }
                targetprim = stage->GetPrimAtPath(targetpath);
                if (targetprim)
                    material = UsdShadeMaterial(targetprim);
            }
            if (!material && targetprim)
            {
                UsdShadeMaterialBindingAPI bindingapi(targetprim);
            ...
            if (material)
            {
                if (!husdBindDirect(material, geoprim,
                        HUSD_BindMaterial::Strength::STRONG,
                        myPurpose))
                    return false;
            }
            else
                HUSD_ErrorScope::addWarning(
                    HUSD_ERR_CANT_FIND_MATERIAL, targetstr.c_str());
        }

        // Optionally remove the usdmaterialpath attribute we used to
        // find the material.
        if (remove_attr)
        {
            SdfPropertySpecHandle attribs_to_remove[] = {
                    attrspec, reffilespec, refprimspec
            };

            if (edit_source_layer)
            {
                auto primspec = layer->GetPrimAtPath(
                    attrspec->GetOwner()->GetPath());
                if (primspec)
                {
                    for (const SdfPropertySpecHandle &attr : attribs_to_remove)
                    {
                        if (attr)
            ...
            else
            {
                for (const SdfPropertySpecHandle &attr : attribs_to_remove)
                {
                    ...
                    UsdAttribute block_attr = geoprim.CreateAttribute(
                            attr->GetNameToken(), SdfValueTypeNames->String);
                    block_attr.Block();
                }
            }
        }

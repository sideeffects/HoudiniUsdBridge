...
bool
HUSD_ConfigurePrims::setAssetThumbnail(const HUSD_FindPrims &findprims,
        const UT_StringRef &thumbnail) const
{
    SdfAssetPath thumbnail_path(thumbnail.toStdString());

    return husdConfigPrim(myWriteLock, findprims, [&](UsdPrim &prim)
        {
            UsdMediaAssetPreviewsAPI previewsapi =
                UsdMediaAssetPreviewsAPI::Apply(prim);

            previewsapi.SetDefaultThumbnails(
                UsdMediaAssetPreviewsAPI::Thumbnails(thumbnail_path));

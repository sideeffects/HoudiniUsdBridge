...
static inline void
husdSaveParmDialogScript( UsdShadeNodeGraph &usd_mat_or_graph, VOP_Node &vop )
{
    PI_EditScriptedParms parms( &vop, /*reserved*/ false, /*autolink*/ false );
    if( parms.getNParms() <= 0 )
        return;

    // Ask for shader parms, even though VOP is a material container.
    // Reusing the shader logic makes it consistent and familiar to the users.
    UT_StringSet shader_parms;
    for( auto &&parm : vop.getUSDShaderParms( /*force_parms_at_defaut=*/ true ))
        shader_parms.insert( parm->getTokenRef() );

    // Remove non-input parms. Iterate back since we are removing members.
    for( int i=parms.getNParms()-1; i >= 0; i-- )
    {
	PI_EditScriptedParm *parm = parms.getParm(i);

	// Keep parameters that correspond to material input attributes.
	if( shader_parms.contains( parm->myName ))
	    continue;

	// Also, need to manage (keep) folder delimeters ourselves.
	if( parm->getIsGroupParm() )
	    continue;

	parms.removeParms(i, i);
    }
    parms.removeEmptyFolders();

    // Save the pruned parm dialog. 
    if( parms.getNParms() > 0 )
    {
        UT_OStringStream ss;
        UT_String warnings;
        ss << "{\n";
        ss << "    name    usdmaterial\n\n";
        parms.save(ss, warnings, /*validate_joins*/ true );
        ss << "\n}";

        usd_mat_or_graph.GetPrim().SetCustomDataByKey( 
                HUSDgetDialogScriptToken(), VtValue( ss.str().toStdString() ));
    }
}

 ...
 *      416-504-9876
 *
 * NAME:    HUSD_FSUsdzReaderHelper.C
 *
 * COMMENTS:
 *
 */
...
{
    new HUSD_FSUsdzReaderHelper();
    new HUSD_FSUsdzInfoHelper();
}

FS_ReaderStream *
HUSD_FSUsdzReaderHelper::createStream(const char *source, const UT_Options *options)
{
    int		len;
...
bool
HUSD_FSUsdzInfoHelper::canHandle(const char* source)
{
    return isValidUsdzAssetFile(source);
}

bool
HUSD_FSUsdzInfoHelper::hasAccess(const char* source, int mode)
{
...
bool
HUSD_FSUsdzInfoHelper::getIsDirectory(const char* source)
{
...
int
HUSD_FSUsdzInfoHelper::getModTime(const char* source)
{
...
int64
HUSD_FSUsdzInfoHelper::getSize(const char* source)
{
...
bool
HUSD_FSUsdzInfoHelper::getContents(const char* source, UT_StringArray& contents,
    UT_StringArray* dirs)
{
    // usdz files are not directories
    return false;

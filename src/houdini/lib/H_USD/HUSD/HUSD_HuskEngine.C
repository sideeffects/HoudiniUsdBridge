...
void
HUSD_HuskEngine::setKarmaRandomSeed(int seed)
{
    myEngine->setKarmaRandomSeed(seed);
}

void
HUSD_HuskEngine::mplayMouseClick(int x, int y) const
{
    myEngine->mplayMouseClick(x, y);
}
...
void
HUSD_HuskEngine::listDelegates(UT_StringArray &cameras)
{
    XUSD_HuskEngine::listDelegates(cameras);
}

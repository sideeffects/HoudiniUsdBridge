...
UT_Matrix4D
HUSD_Info::getXformFromOpOrder(const UT_StringRef &primpath,
        ...
        UT_CameraParms &camparms) const
{
    ...
    float            hap, vap, hoff, voff;
    fpreal64         mpu, open, close;
    GfVec2f          cliprange;

    if (!cam)
        return false;

    cam.GetHorizontalApertureAttr().Get(&hap, tc);
    cam.GetHorizontalApertureOffsetAttr().Get(&hoff, tc);
    cam.GetVerticalApertureAttr().Get(&vap, tc);
    cam.GetVerticalApertureOffsetAttr().Get(&voff, tc);
    getMetrics(upaxis, mpu);

    cam.GetFocalLengthAttr().Get(&camparms.focal, tc);
    camparms.focal = camparms.focal * mpu * 100.0;
    camparms.aperture = hap * mpu * 100.0;
    cam.GetFocusDistanceAttr().Get(&camparms.focus, tc);
    cam.GetFStopAttr().Get(&camparms.fstop, tc);
    cam.GetProjectionAttr().Get(&token, tc);
    if (token == UsdGeomTokens->orthographic)
        camparms.projection = OBJ_PROJ_ORTHO;
    else
        camparms.projection = OBJ_PROJ_PERSPECTIVE;
    // Ignore the focal/aperture in ortho projection
    if (camparms.projection == OBJ_PROJ_ORTHO)
        camparms.focal = camparms.aperture = 1;

    HUSD_Path rspath = getBestRenderSettings();
    ...
    if (rsprim)
    {
        ...
        camparms.xres = res[0];
        camparms.yres = res[1];
        rsprim.GetPixelAspectRatioAttr().Get(&camparms.aspect, tc);
    }
    else
    {
        camparms.xres = 1920;
        camparms.yres = int(fpreal(camparms.xres) * (vap / hap));
        camparms.aspect = 1.0;
    }
    cam.GetClippingRangeAttr().Get(&cliprange, tc);
    camparms.mynear = cliprange[0];
    camparms.myfar = cliprange[1];
    cam.GetShutterOpenAttr().Get(&open, tc);
    cam.GetShutterCloseAttr().Get(&close, tc);
    camparms.shutterTime = close - open;

    // No equivalent to camparms.curvature

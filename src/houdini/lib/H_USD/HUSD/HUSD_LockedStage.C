...
{
    lockStage(data, nodeid, output_index, strip_layers, t, opts);
}
        ...
        const UT_Options &opts)
{
	...
	// adopted by the root layer. It also means that when we save with the
        ...
        // Create this block to contain the SdfChangeBlock.
        {
            SdfChangeBlock changeblock;

            // Add the sublayers to the root layer along with the
            // matching offsets.
	...
	myRootLayerIdentifier = outroot->GetIdentifier();
        ...
        if (CAST_LOPNODE(lop))
            myStageCacheIdentifier = GusdStageCache::
                CreateLopStageIdentifier(lop, output_index,
                    strip_layers, t, opts);
        else

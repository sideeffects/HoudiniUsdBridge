...
bool
HUSD_Merge::addLayer(const UT_StringRef &layer_text)
{
    bool                 success = false;

    if (layer_text.isstring())
    {
        SdfLayerRefPtr layer = HUSDcreateAnonymousLayer();
        if (layer)
        {
            layer->ImportFromString(layer_text.toStdString());
            myPrivate->mySubLayers.append(
                XUSD_LayerAtPath(layer, layer->GetIdentifier()));
            myPrivate->mySubLayerIds.insert(layer->GetIdentifier());
...
HUSD_Merge::execute(HUSD_AutoWriteLock &lock, bool replace_all) const
{
    ...
    if (outdata && outdata->isStageValid())
    {
             ...
             isFlattenIntoActiveLayerStyle(myMergeStyle)))
        {
            ...
            SdfLayerRefPtr flattened = HUSDflattenLayersInChunks(sublayers,
                sublayeroffsets, outdata->stage());

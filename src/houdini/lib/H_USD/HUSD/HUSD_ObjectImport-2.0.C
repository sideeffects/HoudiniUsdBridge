...
HUSD_PathSet
HUSD_ObjectImport2::getOutputShaders(const UT_StringHolder &matpath) const
{
    HUSD_PathSet shaders;

    auto outdata = myWriteLock.data();
    if (!outdata || !outdata->isStageValid())
        return shaders;

    auto prim = outdata->stage()->GetPrimAtPath(HUSDgetSdfPath(matpath));
    if (!prim)
        return shaders;
    ...
    for (auto &&output : mat.GetOutputs())
        for (auto &&source : output.GetConnectedSources())
            shaders.insert(source.source.GetPrim().GetPath());

...
public:
    ...
    bool         eraseWithDescendants(const HUSD_Path &path);
    bool         eraseWithDescendants(const UT_StringRef &path);
    bool         getDescendants(const HUSD_Path &path,
    ...
    // Will throw an exception if the provided python object can't be
    // converted to the data types we want.
    void                         setPythonPaths(void *primpaths);
    ...
    iterator                     end() const;
    iterator                     find(const HUSD_Path &path) const;

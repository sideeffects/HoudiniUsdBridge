...
{
    UT_Array<UtType> unique_values;
    unique_values.setCapacity(values.size());
    indices.setCapacity(values.size());

    for (const UtType &value : values)
    {
        exint found = -1;
        for (exint i = 0, n = unique_values.size(); i < n; ++i)
        {
        ...
        {
            indices.append(unique_values.size());
            unique_values.append(value);
        }
    }
    values = std::move(unique_values);
    values.shrinkToFit();
}

// This mirrors GEO_FilePrimUtils::GEOmatchAttribPattern
bool
husdMatchAttribPattern(
        const UT_StringMMPattern &pattern,
        const UT_StringRef &usd_attr_name,
        const GA_Attribute *attr)
{
    ...
    const bool matched_name
            = usd_attr_name.multiMatch(
                      pattern, &name_explicitly_excluded, &name_match_idx);

    if (!attr)
        return matched_name;

    const GA_StorageClass storage_class = attr->getStorageClass();

    UT_StringRef type_token;
    if (storage_class == GA_STORECLASS_STRING)
        type_token = theStringTypeName.asRef();
    else if (storage_class == GA_STORECLASS_REAL
             && attr->getTypeInfo() == GA_TYPE_TEXTURE_COORD)
...
public:
    ...
    virtual bool             write(HUSD_AutoWriteLock &writelock) const = 0;
};
    ...
    bool                     myIsFirstSample;
                             ...
                             husd_SetAttributeQueue(
                                 ...
                                 : myPrimPath(primpath),
                                   ...
                                   myIsFirstSample(first_sample)
                             {}

    bool                     write(HUSD_AutoWriteLock &writelock) const override
                             {
    ...
    bool                     write(HUSD_AutoWriteLock &writelock) const override
                             {
                                 ...
                                 {
                                     bool ok = setattrs.setPrimvarArray(
                                                    myPrimPath, myPrimvarName,
                                                    myInterpolation, myValues,
                                                    myTimeCode, myValueType,
                                                    1, myIsFirstSample);

                                     ok = ok && setattrs.setPrimvarIndices(
                                                    myPrimPath, myPrimvarName,
                                                    myIndices, myTimeCode,
                                                    myIsFirstSample);
                                     return ok;
                                 }
...
class husd_SetXformQueue final : public husd_UsdWriteQueueBase
{
public:
    UT_StringHolder          myPrimPath;
    UT_Matrix4D              myXform;
    ...
    bool                     write(HUSD_AutoWriteLock &writelock) const override
                             {
                                 ...
                                 HUSD_FindPrims findprims(writelock, myPrimPath);
                                 if (!xformer.addXform(findprims, "", myXform, myTimeCode))
                                 {
                                     HUSD_ErrorScope::addWarning(
                                           HUSD_ERR_CANT_SET_PROPERTY, "Xform");
                                     return false;
                                 }
                                 return true;
                             }
...
bool _copySopAttrToUsdPrimvar(HUSD_AutoReadLock &input_readlock,
                              const GU_Detail *gdp,
                              ...
                              bool forceupdate = false)
{
    ...
    const GA_Attribute *attr = gdp->findAttribute(GA_ATTRIB_POINT, attrname);
    if (husdMatchAttribPattern(config.myIndexedPrimvarsPattern, usdname, attr))
    {
        UT_ExintArray indices;
        husdMakeIndexed(values, indices);
        pending.appendPrimvar(primpath, usdname,
            HUSD_Constants::getInterpolationVarying(),
            timecode, valuetype, std::move(values), std::move(indices));
    }
    else
    {
        pending.appendPrimvar(primpath, usdname,
...
bool copySopAttrToUsdPrimvar(HUSD_AutoReadLock &input_readlock,
                             const GU_Detail *gdp,
                             const GA_Range &primrange,
                             const UT_StringRef &primpath,
                             const UT_StringRef &attrname,
        ...
        if (storage == GA_STORE_REAL32)
        {
            if (tuplesize == 16)
                return _copySopAttrToUsdPrimvar<UT_Matrix4F>(
                           input_readlock, gdp, primrange, primpath, attrname,
                           timecode, valuetype, map, config, pending, forceupdate);
...
template <class UtType>
void _copySopAttrToUsdAttr(HUSD_AutoReadLock &input_readlock,
                           const GU_Detail *gdp,
                           const GA_Range &primrange,
                           const UT_StringRef &primpath,
                           const UT_StringRef &attrname,
...
void _updateTransformAttrs(HUSD_AutoReadLock &input_readlock,
                           const GU_Detail *gdp,
                           const GA_Range &primrange,
                           const UT_StringRef &primpath,
                           const HUSD_TimeCode &timecode,
                           ...
                           husd_UsdWriteQueue &pending)
{
    ...
    if (usdxform_attr.isValid())
    {
        ...
        if (!world_xform.isEqual(info.getWorldXform(primpath, timecode)))
        {
            ...
            pending.appendXform(primpath, delta, timecode);
        }
    ...
    if (existing)
    {
            ...
            copystyle == HUSD_PointInstancerCopyStyle::Update)
        {
            ...
            if (protoindices.isEmpty())
            {
                ...
                // the array should have one entry for each existing instance
                HUSD_Info info(input_readlock);
                protoindices.appendMultiple(0, offsetmap.myOriginalNumInstances);
            }
            // make room for new ids / instances
            protoindices.appendMultiple(0, offsetmap.myNewIds.size());
        }
        else
    ...
    else
    {
        ...
        if (idshandle.isValid())
        {
            ...
            UTparallelFor(GA_SplittableRange(primrange),
                ...
                {
                    for (GA_Offset ptoff : splitrange)
                        // check the offsetmap for id, not the idshandle directly
                        // since the attr value may be -1.
                        ids[offsetmap.getIdx(ptoff)] = offsetmap.getId(ptoff);
                });
        }
        else
        {
...
void _updateInvisIds(HUSD_AutoReadLock &input_readlock,
                     const GU_Detail *gdp,
                     const GA_Range &primrange,
                     const UT_StringRef &primpath,
                     const HUSD_TimeCode &timecode,
...
HUSD_PointInstancerSampleData::~HUSD_PointInstancerSampleData() = default;
    ...
    for (const auto &data : map)
    {
        ...
        [&]{
            ...
            if (info.hasAuthoredValueForProperty(data.first,
                ...
                _updateTransformAttrs<UT_QuaternionF>(input_readlock, gdp, primrange,
                    data.first, myTimeCode, data.second, config, *myWriteQueue);
            else
                _updateTransformAttrs(input_readlock, gdp, primrange, data.first,
            ...
            UTparallelFor(updaterange,
                [&](const UT_BlockedRange<exint> &subrange)
                {
                    ...
                    for (exint idx = subrange.begin(), end = subrange.end();
                         ...
                         ++idx)
                    {
                        copySopAttrToUsdPrimvar(input_readlock, gdp, primrange,
                                                data.first,
                                                updatePrimvarNames[idx],
                                                myTimeCode, data.second,
                                                config, *myWriteQueue, true);
                    }
...
void
HUSD_PointInstancerSampleData::apply(HUSD_AutoWriteLock &writelock)
{
    myWriteQueue->writeAll(writelock);
}

...
namespace
{
    static constexpr UT_StringLit       theRaster("raster");
    ...
    public:
        ...
        const char      *overrideProductName(
                                const XUSD_RenderProduct &p) const override;
        const char      *overrideSnapshotPath(
                                const XUSD_RenderProduct &p) const override;
        const char      *overrideSnapshotSuffix(
                                const XUSD_RenderProduct &p) const override;
...
#define DECL_TOKEN(NAME) \
    ...
    DECL_TOKEN(instantaneousShutter);
...
bool
husd_RenderSettingsContext::overrideInstantaneousShutter(bool v) const
{
    return myImpl->overrideInstantaneousShutter(v);
}
...
husd_RenderSettingsContext::overrideProductName(
        const XUSD_RenderProduct &xp) const
{
    const auto    *hp = dynamic_cast<const husd_RenderProduct *>(&xp);
    return hp ? myImpl->overrideProductName(*hp->myImpl) : nullptr;
}
...
{
    const auto    *hp = dynamic_cast<const husd_RenderProduct *>(&xp);
    return hp ? myImpl->overrideSnapshotPath(*hp->myImpl) : nullptr;
}
...
{
    const auto    *hp = dynamic_cast<const husd_RenderProduct *>(&xp);
    return hp ? myImpl->overrideSnapshotSuffix(*hp->myImpl) : nullptr;
}
...
HUSD_RenderProduct::expandProduct(
        ...
        int frame)
{
    return myOwner->expandProduct(ctx.impl(), frame);
}
#endif
        ...
        bool delegate_products)
{
    ...
    if (mkdirs)
    {
        ...
        for (const auto *prod : prods)
        {
            ...
            if (prod->productType() == theRaster.asRef() ||
...
bool
HUSD_RenderSettings::instantaneousShutter(const HUSD_RenderProduct *p) const
{
    return myOwner->instantaneousShutter(xusdProduct(p));
}

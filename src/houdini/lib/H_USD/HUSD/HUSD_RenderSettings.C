...
namespace
{
    ...
    public:
        ...
        const char      *overrideSnapshotSuffix(const XUSD_RenderProduct &p,
                                int pidx) const override;
        UT_StringHolder  addTileSuffix(const UT_StringHolder &n) const override;
    ...
    static UT_StringHolder toStr(const std::string &v) { return UT_StringHolder(v); }
    static UT_StringHolder toStr(const TfToken &v) { return UT_StringHolder(v.GetText()); }
    static UT_StringHolder toStr(const SdfPath &v) { return HUSD_Path(v).pathStr(); }
    static UT_StringHolder toStr(const SdfAssetPath &v)
    {
        const std::string       &r = v.GetResolvedPath();
        if (r.empty())
            return UT_StringHolder(v.GetAssetPath());
        return UT_StringHolder(r);
    }

    static bool
    toStr(UT_StringHolder &dest, const VtValue &v)
    {
#define STRING_OF(TYPE) \
        if (v.IsHolding<TYPE>()) { \
            dest = toStr(v.UncheckedGet<TYPE>()); \
            return true; \
        } \
        /* end macro */
        STRING_OF( std::string );
        STRING_OF( TfToken );
        STRING_OF( SdfPath );
        STRING_OF( SdfAssetPath );
#undef STRING_OF
        UT_OStringStream        os;
        os << v;
        dest = std::move(os.str());
        return true;
    }

    template <typename T>
    static void
    extractStrings(UT_StringArray &dest, const T *data, exint size)
    {
        dest.setSize(size);
    ...
    static bool
    toStrArray(UT_StringArray &dest, const VtValue &v)
    {
#define ARRAY_OF(TYPE) \
        if (v.IsHolding<TYPE>()) { \
            const auto &arr = v.UncheckedGet<TYPE>(); \
            extractStrings(dest, arr.data(), arr.size()); \
            return true; \
        } \
        /* end macro */
        ARRAY_OF( VtArray<std::string> );
        ARRAY_OF( VtArray<TfToken> );
        ARRAY_OF( VtArray<SdfPath> );
        ARRAY_OF( SdfPathVector );
#undef ARRAY_OF
        UT_StringHolder tmp;
        if (toStr(tmp, v))
        {
    ...
    template <typename T>
    static void
    extractScalar(UT_Array<fpreal64> &dest, const T *data, exint size)
    {
        dest.setSize(size);
        std::copy(data, data+size, dest.data());
    }

    template <typename T>
    static void
    extractVector(UT_Array<fpreal64> &dest, const T *data, exint size)
    {
        exint   nf = size * T::dimension;
        extractScalar(dest, reinterpret_cast<const typename T::ScalarType *>(data), nf);
    }
    ...
    static bool
    toFloatArray(UT_Array<fpreal64> &dest, const VtValue &v)
    {
#define ARRAY_OF_SCALAR(TYPE) \
        if (v.IsHolding<TYPE>()) { \
            const auto &arr = v.UncheckedGet<TYPE>(); \
            extractScalar(dest, arr.data(), arr.size()); \
            return true; \
        } \
        /* end macro */
        ARRAY_OF_SCALAR( VtArray<fpreal32> )
        ARRAY_OF_SCALAR( VtArray<fpreal64> )
        ARRAY_OF_SCALAR( VtArray<fpreal16> )
        ARRAY_OF_SCALAR( VtArray<bool> )
        ARRAY_OF_SCALAR( VtArray<int32> )
        ARRAY_OF_SCALAR( VtArray<int64> )
        ARRAY_OF_SCALAR( VtArray<uint32> )
        ARRAY_OF_SCALAR( VtArray<uint64> )
#undef ARRAY_OF_SCALAR
#define ARRAY_OF_VECTOR(TYPE) \
        if (v.IsHolding<TYPE>()) { \
            const auto &arr = v.UncheckedGet<TYPE>(); \
            extractVector(dest, arr.data(), arr.size()); \
            return true; \
        } \
        /* end macro */
        ARRAY_OF_VECTOR( VtArray<GfVec2f> )
        ARRAY_OF_VECTOR( VtArray<GfVec2d> )
        ARRAY_OF_VECTOR( VtArray<GfVec2h> )
        ARRAY_OF_VECTOR( VtArray<GfVec2i> )
        ARRAY_OF_VECTOR( VtArray<GfVec3f> )
        ARRAY_OF_VECTOR( VtArray<GfVec3d> )
        ARRAY_OF_VECTOR( VtArray<GfVec3h> )
        ARRAY_OF_VECTOR( VtArray<GfVec3i> )
        ARRAY_OF_VECTOR( VtArray<GfVec4f> )
        ARRAY_OF_VECTOR( VtArray<GfVec4d> )
        ARRAY_OF_VECTOR( VtArray<GfVec4h> )
        ARRAY_OF_VECTOR( VtArray<GfVec4i> )
#undef ARRAY_OF_VECTOR

        fpreal64        scalar;
        if (getValue<fpreal64, fpreal32, fpreal64, bool, int32, int64>(scalar, v))
        {
            dest.append(scalar);
            return true;
        }
    ...
    static bool
    mapLookup(const HdAovSettingsMap &map, const char *name, UT_Array<fpreal64> &val)
    {
        auto it = map.find(TfToken(name));
        if (it == map.end())
            return false;
        return toFloatArray(val, it->second);
    }
    static bool
    mapLookup(const HdAovSettingsMap &map, const char *name, UT_StringArray &val)
    {
        auto it = map.find(TfToken(name));
        if (it == map.end())
            return false;
        return toStrArray(val, it->second);
    }
...
{
    const HUSD_HuskImaging	*engine = myImpl->huskEngine();
    if (!engine)
        return HdAovDescriptor();
    HdAovDescriptor     desc;
    engine->defaultAOVDescriptor(aov.GetText(), desc);
    return desc;
}
...
UT_StringHolder
husd_RenderSettingsContext::addTileSuffix(const UT_StringHolder &n) const
{
    return myImpl->addTileSuffix(n);
}
...
bool
HUSD_RenderVar::lookup(const char *token, UT_Array<fpreal64> &val) const
{
    return mapLookup(myOwner->desc().aovSettings, token, val);
}

bool
HUSD_RenderVar::lookup(const char *token, UT_StringArray &val) const
{
    return mapLookup(myOwner->desc().aovSettings, token, val);
}
...
bool
HUSD_RenderProduct::lookup(const char *token, UT_Array<fpreal64> &val) const
{
    return mapLookup(myOwner->settings(), token, val);
}

bool
HUSD_RenderProduct::lookup(const char *token, UT_StringArray &val) const
{
    return mapLookup(myOwner->settings(), token, val);
}
...
UT_StringHolder
HUSD_RenderProduct::fullOutputName() const
{
    return myOwner->fullOutputName();
}

UT_StringHolder
HUSD_RenderProduct::insertExtraBeforeExtension(const UT_StringHolder &filename,
                                        const char *extra,
                                        const char *path)
{
    return XUSD_RenderProduct::insertExtraBeforeExtension(filename, extra, path);
}
...
namespace
{
    ...
    static void
    addImageMetaData(IMG_FileParms &fparms, const MAP_T &settings)
    {
...
{
    addImageMetaData(fparms, myOwner->settings());
}
...
bool
HUSD_RenderSettings::init(const HUSD_HuskImaging &engine,
        const UT_StringHolder &settings_path,
        HUSD_RenderSettingsContext &ctx)
{
    return engine.initSettings(*myOwner, settings_path, ctx);
}

bool
HUSD_RenderSettings::getKarmaRandomSeed(int64 &seed,
        const HUSD_RenderSettingsContext &ctx) const
{
    return myOwner ? myOwner->getKarmaRandomSeed(seed, ctx.impl()) : false;
}

bool
HUSD_RenderSettings::makeDirPathDirs(const char *dir)
{
    if (!UTisstring(dir))
        return true;
...
{
    HUSD_HuskImaging	*engine = SYSconst_cast(ctx.huskEngine());
    UT_ASSERT(engine);
    if (!engine->updateSettings(*myOwner, ctx, custom_product_action))
    {
        return false;
    }
    ...
    if (!expandProducts(ctx, frame, product_group, delegate_products))
        return false;
...
HUSD_RenderSettings::expandProducts(const HUSD_RenderSettingsContext &ctx,
        int fnum, int product_group, bool delegate_products)
{
    return myOwner->expandProducts(ctx.impl(), fnum,
            product_group, delegate_products);
}
...
bool
HUSD_RenderSettings::resolveProducts(const HUSD_HuskImaging &engine,
        HUSD_RenderSettingsContext &ctx,
        HUSD_CustomProductAction custom_product_action)
{
    return engine.resolveProducts(*myOwner, ctx, custom_product_action);
}
...
bool
HUSD_RenderSettings::lookup(const char *token, UT_Array<fpreal64> &val) const
{
    return mapLookup(myOwner->renderSettings(), token, val);
}

bool
HUSD_RenderSettings::lookup(const char *token, UT_StringArray &val) const
{
    return mapLookup(myOwner->renderSettings(), token, val);
}
...
void
HUSD_RenderSettings::addMetaData(IMG_FileParms &fparms) const
{
    addImageMetaData(fparms, myOwner->renderSettings());
}

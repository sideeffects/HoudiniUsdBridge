...
public:
    ...
    virtual const HUSD_HuskImaging	*huskEngine() const { return nullptr; }
    ...
    /// When composing an image with tiles, this method allows the context to
...
public:
    ...
    bool        lookup(const char *token, UT_Array<fpreal64> &val) const;
    bool        lookup(const char *token, UT_StringArray &val) const;
    /// @}
...
public:
    ...
    UT_StringHolder     fullOutputName() const; // Product name without tile suffix
    /// @}

    /// Utility function to insert @c extra text just before the filename
    /// extension.  For example "_part".
...
public:
    ...
    // Create a directory (the path is the directory path, not the file path)
    static bool makeDirPathDirs(const char *dir_path);
    ...
    /// initialization.
    bool        init(const HUSD_HuskImaging &engine,
                    const UT_StringHolder &settings_path,
                    HUSD_RenderSettingsContext &ctx);

    /// Check to see if the karma random seed property is authored on the prim
    ...
    bool        resolveProducts(const HUSD_HuskImaging &engine,
                    HUSD_RenderSettingsContext &ctx,

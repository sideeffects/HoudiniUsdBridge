...
public:
    ...
    void addToMaterialBindThreadData(const UsdPrim &prim);
...
void
XUSD_ValidationTaskData::addToMaterialBindThreadData(const UsdPrim &prim)
{
    TfToken mtlrelation("material:binding");
    TfToken mtlfull("material:binding:full");
    TfToken mtlpreview("material:binding:preview");

    if((prim.GetRelationship(mtlrelation).IsValid() ||
        prim.GetRelationship(mtlfull).IsValid()     ||
        prim.GetRelationship(mtlpreview).IsValid())
        && !prim.HasAPI<UsdShadeMaterialBindingAPI>())
    {
        ...
        if(!threadData)
            threadData = new XUSD_ValidationTaskThreadData;
        threadData->myValidationErrors[prim.GetPath()] =
                HUSD_SceneDoctor::MISSING_MATERIAL_BINDING_API;
    } 
}
        ...
        const UsdPrim &prim, bool *prune)
{
    auto validatePrim = [this] (const UsdPrim &prim)
    {
        ...
        if (myFlags.myValidateMaterialBind)
            addToMaterialBindThreadData(prim);
    };

...
enum HUSD_PrimTraversalDemands {
    ...
    // A combination of the default demands and support for instance proxies.
    // This is a very common combination.
    HUSD_TRAVERSAL_DEFAULT_WITH_PROXIES = HUSD_TRAVERSAL_ALLOW_INSTANCE_PROXIES,
};
...
enum HUSD_OverridesLayerId {
    ...
    HUSD_OVERRIDES_VIEWPORT_STATE_LAYER = 1,
    HUSD_OVERRIDES_EXPANSION_LAYER = 2,
    HUSD_OVERRIDES_PURPOSE_LAYER = 3,
    HUSD_OVERRIDES_SOLO_LIGHTS_LAYER = 4,
    HUSD_OVERRIDES_SOLO_GEOMETRY_LAYER = 5,
    HUSD_OVERRIDES_SELECTABLE_LAYER = 6,
    HUSD_OVERRIDES_BASE_LAYER = 7
};
#define HUSD_OVERRIDES_NUM_LAYERS 8
...
// Enum describing possible aspect ratio conformance policies.
...
// Describes the different ways of dealing with custom RenderProduct types:
// IGNORE:              Do nothing, even if all products are custom types.
// ADD_AOVS:            Request AOVs associated with custom products from the
//                      renderer. This is used for viewport renders.
// CREATE_DUMMY_RASTER: If all products are custom, create a dummy raster
//                      product to at least output a beauty image.
enum HUSD_CustomProductAction {
...
// Hashing function for render keys.
...
// Map of render key to the scene graph path it corresponds to.
typedef UT_Map<HUSD_RenderKey, UT_StringHolder> HUSD_RenderKeyPathMap;
...
// Returns true if the given path is a syntactibally valid USD path.
HUSD_API bool
...
// are replaced with underscores. Returns true (and adds a warning if
// addwarnings is true) if the string was modified.
HUSD_API bool
...
// Splits a selection path into separate primitive path and instance id
// components. Ignores any ":prototype" suffix on the instance id.
// Return true if an instance id was found, otherwise return false and
// set primpath = selectionpath (and instanceid = -1). This is just like
// the loputils.splitInstanceIdAndPath python function.
HUSD_API bool
...
/// Return an array of layer ids that excludes the "expansion" layer. Most
/// layer operations should, by default, ignore the expansion layer as it is
/// controlled by the scene graph expansion state, not through direct user
/// interaction. The viewport state layer is excluded from this list because
/// we don't want those edits to show modifications in the scene graph tree.
HUSD_API const UT_Array<HUSD_OverridesLayerId> &
...
// Returns an array of paths that conflict with existing relocates
HUSD_API UT_StringArray HUSDgetConflictingRelocates(
        const HUSD_AutoAnyLock &lock,

...
public:
    ...
    virtual void	addAndSetShaderAttrib( UsdShadeConnectableAPI &shader,
...
protected:  
    ...
    UsdAttribute	addShaderParmAttrib( UsdShadeConnectableAPI &api,
    ...
    UsdAttribute	addShaderParmAttrib( UsdShadeShader &shader,
				...
				const SdfValueTypeName &type ) const
                        {
                            auto connectable_api( shader.ConnectableAPI() );
                            return addShaderParmAttrib( 
                                    connectable_api, name, type );
                        }
};
...
UsdAttribute
husd_ParameterTranslator::addShaderParmAttrib( UsdShadeConnectableAPI &api,
	const UT_StringRef &name, const SdfValueTypeName &type ) const
{
    ...
    return api.CreateInput( TfToken( name.toStdString() ), type );
}
...
{
public:
    void            addAndSetShaderAttrib( UsdShadeConnectableAPI &shader,
			const HUSD_TimeCode &time_code,
			const PRM_Parm &def_parm, 
			const PRM_Parm *val_parm = nullptr,
...
husd_SimpleParameterTranslator::addAndSetShaderAttrib( 
	UsdShadeConnectableAPI &shader, const HUSD_TimeCode &time_code,
...
private:
    UsdAttribute     addAndSetRampBasisAttrib( UsdShadeConnectableAPI &shader,
			    const UT_StringRef &name, 
			    const UT_Ramp &ramp_val ) const;
    UsdAttribute     addAndSetRampKeysAttrib( UsdShadeConnectableAPI &shader,
			    const UT_StringRef &name, 
			    const UT_Ramp &ramp_val ) const;
    UsdAttribute     addAndSetRampValuesAttrib( UsdShadeConnectableAPI &shader,
			    const UT_StringRef &name, 
...
husdRampPositionsName( const PRM_Parm &parm )
{
    const PRM_SpareData *spare = parm.getSparePtr();
    UT_StringHolder name( spare ? spare->getRampKeysVar() : "" );
    if( !name )
    {
        UT_WorkBuffer buffer;
	buffer.sprintf( "%s_the_key_positions", parm.getToken() );
        name = UT_StringHolder( std::move(buffer) );
    }
    return name;
}

static inline UT_StringHolder
husdRampValuesName( const PRM_Parm &parm )
{
    const PRM_SpareData *spare = parm.getSparePtr();
    UT_StringHolder name( spare ? spare->getRampValuesVar() : "" );
    if( !name )
    {
        UT_WorkBuffer buffer;
	buffer.sprintf( "%s_the_key_values", parm.getToken() );
        name = UT_StringHolder( std::move(buffer) );
    }
    return name;
}
...
static inline UT_StringHolder
husdRampBasisName( const PRM_Parm &parm )
{
    const PRM_SpareData *spare = parm.getSparePtr();
    UT_StringHolder name( spare ? spare->getRampBasisVar() : "" );
    if( !name )
    {
        UT_WorkBuffer buffer;  // See VOP_ParmRamp::getShaderParmNames()
	buffer.sprintf( "%s_the_basis_strings", parm.getToken() );
        name = UT_StringHolder( std::move(buffer) );
    }
    return name;
}
	...
	const UT_IntArray &dependent_node_ids ) const
{
    ...
    auto keys_name  = husdRampPositionsName( def_parm );
    auto values_name = husdRampValuesName( def_parm );
    auto basis_name = husdRampBasisName( def_parm );
...
protected:
    ...
    /// @{ Utility functions
    UsdPrim	        getUsdPrim( const UT_StringRef &prim_path ) const;
    ...
    void		authorUsdInput( UsdShadeConnectableAPI &api,
				const PRM_Parm &def_parm, 
				const PRM_Parm *val_parm = nullptr ) const;
    void		authorUsdInput( UsdShadeShader &shader,
				const PRM_Parm &def_parm, 
				const PRM_Parm *val_parm = nullptr ) const
                        {
                            auto connectable_api( shader.ConnectableAPI() );
                            authorUsdInput(connectable_api, def_parm, val_parm);
                        }
    ...
    void                createAndConnectAncestorRampInput( 
                                const UT_StringRef &usd_material_path,
                                const UT_StringRef &usd_parent_path,
                                UsdShadeShader &shader,
                                VOP_Node &vop, int input_idx, 
                                VOP_ParmGenerator *ramp_vop ) const;
...
UsdPrim
husd_ShaderTranslatorHelper::getUsdPrim( const UT_StringRef &path ) const
{
    auto outdata = myWriteLock.data();
    if( outdata && outdata->isStageValid() )
        return outdata->stage()->GetPrimAtPath(SdfPath( path.toStdString() ));
...
void
husd_ShaderTranslatorHelper::authorUsdInput( UsdShadeConnectableAPI &api,
	const PRM_Parm &def_parm, const PRM_Parm *val_parm ) const
{
    const husd_ParameterTranslator *parm_xtor = getParmTranslator( def_parm );
    if( parm_xtor )
	parm_xtor->addAndSetShaderAttrib( api, myTimeCode, def_parm, 
...
static inline void
husdConnectAncestorRampComponent( 
        UsdShadeShader &shader,
        const UT_StringRef &shader_input_name,
        const UsdShadeConnectableAPI &ancestor,
        const UT_StringRef &ancestor_input_name )
{
    auto ancestor_input = ancestor.GetInput( 
            TfToken( ancestor_input_name.toStdString() ));
    auto shader_input = shader.CreateInput( 
            TfToken( shader_input_name.toStdString() ),
            ancestor_input.GetTypeName() );
    UsdShadeConnectableAPI::ConnectToSource( 
            shader_input, ancestor_input );
}

void
husd_ShaderTranslatorHelper::createAndConnectAncestorRampInput( 
        const UT_StringRef &usd_material_path,
        const UT_StringRef &usd_parent_path,
        UsdShadeShader &shader,
        VOP_Node &vop, int input_idx, 
        VOP_ParmGenerator *ramp_vop ) const
{
    ...
    // First find the ramp parameter on an ancestor or shader VOP node.
    PRM_Parm *value_parm = nullptr;
    const UT_StringHolder &ancestor_input_name = ramp_vop->getParmNameCache();
    bool is_color = (ramp_vop->evalInt("ramptype", 0, 0.0f) == 0);
    OP_Node  *container_node = vop.getParent();
    if( container_node )
	value_parm = container_node->getParmPtr( ancestor_input_name );
    if( !value_parm )
	value_parm = ramp_vop->getParmPtr( 
                is_color ? "rampcolordefault" : "rampfloatdefault" );

    // Author the ramp input attribs on the ancestor primitive.
    UsdShadeConnectableAPI ancestor( getUsdPrim(  
            ramp_vop->isSubnetInput() ? usd_parent_path : usd_material_path ));
    authorUsdInput( ancestor, *value_parm );

    // Connect the shader inputs to ancestor inputs.
    UT_String shader_parm_name;
    vop.getParmNameFromInput( shader_parm_name, input_idx );
    PRM_Parm *shader_parm = vop.getParmPtr( shader_parm_name );
    if( shader_parm )
    {
        husdConnectAncestorRampComponent( 
                shader, husdRampPositionsName( *shader_parm ),
                ancestor, husdRampPositionsName( *value_parm ));
        husdConnectAncestorRampComponent( 
                shader, husdRampValuesName( *shader_parm ),
                ancestor, husdRampValuesName( *value_parm ));
        husdConnectAncestorRampComponent( 
                shader, husdRampBasisName( *shader_parm ),
                ancestor, husdRampBasisName( *value_parm ));
    }
}
...
{
    OP_Operator *op = parm_vop->getOperator();
    if( op && op->getName() == VOP_PARMRAMP_NODE_NAME )
    {
        createAndConnectAncestorRampInput( usd_material_path, usd_parent_path,
                shader, vop, input_idx, parm_vop );
    }
    else
    {
        ...
        auto shader_input = husdCreateShaderInput( shader, vop, input_idx);
...
husdGetUSDShaderName( VOP_Node &vop, VOP_Type shader_type, bool is_auto_shader )
{
    ...
    UT_String name_str;
    buff.stealIntoString(name_str);
    HUSDmakeValidUsdName( name_str, false );
	...
	UsdShadeShader &shader, VOP_Node &child_vop ) const
{
	...
	authorUsdInput( shader, *child_parm, parent_parm );
    }

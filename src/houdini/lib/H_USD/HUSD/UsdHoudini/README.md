...
## Using schema from Python

`usdGenSchema` will automatically add a `TF_WRAP(...)` entry for your schema in `generatedSchema.module.h`, which is included from `module.cpp`.

This enables using your schema in Python, for example:
```python

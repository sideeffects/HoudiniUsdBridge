...
namespace
{
    ...
    {
    public:
        xusd_FindImageablePrimPaths(const HUSD_TimeCode &timecode)
            : XUSD_FindPrimPathsTaskData(timecode)
        { }
...
XUSD_ExistenceTracker::collectNewStageData(const UsdStageRefPtr &newstage)
{
    ...
    xusd_FindImageablePrimPaths data(
        HUSD_TimeCode(myNewTimeCode.GetValue(), myNewTimeCode.IsDefault()));

    myNewPaths.clear();
    XUSDfindPrims(root, data, predicate);
    data.gatherDataFromThreads(myNewPaths, nullptr);
}

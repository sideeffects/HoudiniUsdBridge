        ...
        int product_group)
{
    myRenderIndex->GetRenderDelegate()->SetRenderSetting(
            HusdHuskTokens->delegateRenderProducts,
...
//----------------------------------------------------------------------------
void
XUSD_HuskEngine::setRenderSetting(const TfToken &token, const VtValue &val)
{
    myRenderIndex->GetRenderDelegate()->SetRenderSetting(token, val);
}

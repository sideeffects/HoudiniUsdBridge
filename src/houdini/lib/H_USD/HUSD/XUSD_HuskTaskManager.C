...
namespace
{
    static const TfToken theKarmaTask(
                                "karmaTask", TfToken::Immortal);
    static const TfToken theRenderBufferDescriptor(
				"renderBufferDescriptor", TfToken::Immortal);
    static const TfToken theRenderPassState(
                                "renderPassState", TfToken::Immortal);
    static const TfToken theAovBindings(
                                "aovBindings", TfToken::Immortal);
        ...
        HdRenderIndex* renderIndex)
    {
        ...
        (*ctx)[theRenderPassState] = VtValue(_renderPassState);
    }
    ...
    XUSD_HuskRenderSetupTask::_PrepareAovBindings(HdTaskContext* ctx, HdRenderIndex* renderIndex)
    {
        ...
        if (!_aovBindings.empty()) {
            ...
            (*ctx)[theAovBindings] = VtValue(_aovBindings);
        }
    }
...
{
    myRenderTaskId = GetControllerId().AppendChild(theKarmaTask);
	...
	const HdAovDescriptorList &outputDescs)
{
    ...
    for (size_t i = 0; i < names.size(); ++i)
    {
        ...
        myTaskDelegate.SetParameter(aovId, theRenderBufferDescriptor, desc);
...
XUSD_HuskTaskManager::SetRenderViewport(const GfVec4d &viewport)
{
	...
	{
            desc.dimensions = dimensions;
            myTaskDelegate.SetParameter(id, theRenderBufferDescriptor, desc);

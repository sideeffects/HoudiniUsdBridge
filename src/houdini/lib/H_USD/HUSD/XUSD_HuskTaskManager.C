...
{
    static void
    dumpAOVs(const HdRenderPassAovBindingVector aovs, const char *msg = "")
    {
#if UT_ASSERT_LEVEL > 0
        UTdebugFormat("== {} AOV Bindings[{}] ==", msg, aovs.size());
        for (int i = 0, n = aovs.size(); i < n; ++i)
        {
            UTdebugFormat("{}: {} {} {}", i, aovs[i].aovName,
                    aovs[i].renderBuffer, aovs[i].renderBufferId);
            for (const auto &s : aovs[i].aovSettings)
                UTdebugFormat("   '{}' := '{}'", s.first, s.second);
        }
#endif
    }
        ...
        XUSD_HuskRenderTaskParams const &params)
    {
        ...
        //dumpAOVs(_aovBindings, "SyncParams");
    }

...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (blendShapeWeights)
    (skinningXforms)
    (skinningScaleXforms) // For dual-quaternion skinning
    (skinningDualQuats) // For dual-quaternion skinning
    (skelLocalToWorld)
        ...
        HdSceneDelegate *scene_delegate)
{
    ...
    static const UT_Set<TfToken> theCompleteSet ({
    ...
    // We don't support GPU dual-quaternion skinning, so these inputs are
    // ignored.
    static const UT_Set<TfToken> theIgnoreSet ({
            _skinningTokens->skinningScaleXforms,
            _skinningTokens->skinningDualQuats
    });
    ...
    {
        if (!theIgnoreSet.contains(token))
    ...
    if (cur_set == theCompleteSet)
    {
        mySkinning = true;
        return mySkinning;
    }
    UT_ASSERT_MSG(
            cur_set.empty() || theCompleteSet == cur_set,
        ...
        VtVec3fArray &rest_points)
{
    ...
    myRestBounds.initBounds();
    myRestPointsAttr->enlargeBounds(myRestBounds);
}

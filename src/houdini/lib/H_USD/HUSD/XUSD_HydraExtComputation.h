...
public:
    ...
    /// We don't have a good way to return approximate bounds for the skinned
    /// result, so for now we just use the rest points' extents (which are
    /// inaccurate if the skeleton has root motion). In the future we need a way
    /// to access the SkelRoot's authored extents.
    /// https://openusd.org//dev/api/_usd_skel__schemas.html#UsdSkel_SkelRoot
    const UT_BoundingBox &getRestBounds() const { return myRestBounds; }

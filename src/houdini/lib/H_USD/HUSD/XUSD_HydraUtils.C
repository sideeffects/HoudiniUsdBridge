...
GT_DataArrayHandle attribGT(const VtValue &value, GT_Type tinfo, int64 data_id)
{
    ...
    if(value.IsHolding<VtVec4hArray>())
        attr = createGTArray(value.Get<VtVec4hArray>(), tinfo, data_id);
    else if(value.IsHolding<VtVec3hArray>())
        attr = createGTArray(value.Get<VtVec3hArray>(), tinfo, data_id);
    else if(value.IsHolding<VtVec2hArray>())
        attr = createGTArray(value.Get<VtVec2hArray>(), tinfo, data_id);
	...
	attr = createGTArray(value.Get<VtVec2dArray>(), tinfo, data_id);
        ...
        attr = createGTArray(value.Get<VtQuathArray>(), tinfo, data_id);
        ...
        attr = createGTArray(value.Get<VtArray<GfHalf> >(), tinfo, data_id);
                  ...
                  const UT_StringRef &name)
{
    ...
    {
        // Workaround an issue where USD holds int values as float. This breaks
        // GLSL because floats cannot be implicitly assigned to ints.
        if(options.hasOption(name) &&
           options.getOptionType(name) == UT_OPTION_INT)
        {
            options.setOptionI(name, int32(value.Get<float >()));
        }
        else
    ...
    else if(value.IsHolding<double>())
    {
        if(options.hasOption(name) &&
           options.getOptionType(name) == UT_OPTION_INT)
        {
            options.setOptionI(name, int64(value.Get<double>()));
        }
        else

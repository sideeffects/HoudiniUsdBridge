...
bool
XUSD_PathSet::eraseWithDescendants(const SdfPath &path)
{
    bool changed = erase(path);

    for (auto it = lower_bound(path); it != end();)
    {
        if ((*it).HasPrefix((path)))
        {
            it = erase(it);
...
bool
XUSD_PathSet::getDescendants(const SdfPath &path,

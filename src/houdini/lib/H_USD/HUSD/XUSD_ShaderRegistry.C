	...
	UT_StringArray &input_names) 
{
    ...
    auto inputs = sdr_node->GetShaderInputNames();
	...
	UT_StringHolder *label)
{
    ...
    if (type)
    {
	auto type_indicator = sdr_input->GetTypeAsSdfType();
        *type = (type_indicator.HasSdfType())
            ? type_indicator.GetSdfType()
            : SdfSchema::GetInstance().FindType(type_indicator.GetSdrType());
    }

    if (default_value)
	*default_value = sdr_input->GetDefaultValueAsSdfType();
